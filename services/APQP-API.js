...
export var API_URL = "https://omnex.ewqims.com/APQPAPI/"; //Edited_by_Sudha
export var DP_API_URL = "http://1.22.172.236/DPAPI/api/"; // Static IP - DocPro
export var WEBAPI = "http://1.22.172.236/APQPwebAPI/"; // Web API - Abinesh

export default {
  setServerUrl(serverUrl) {
    console.log("set serverUrl", serverUrl);
    API_URL = serverUrl;
    var defaultDomain = DP_API_URL.replace("http://", "")
      .replace("https://", "")
      .split(/[/?#]/)[0];
    var actualDomain = serverUrl.replace("http://", "").replace("https://", "");
  ...
  getRegistrationList(
    ...
    console.log(
      "API---------2-------->",
      API_URL + "RegisterDevice/DeviceStatus"
    );
  ...
  getCalendarapi(UserID, SiteID, token, TodayTask, cb) {
    ...
    console.log("calendar formData", formData);

    console.log("Today_Task_API---->" + API_URL + "Calender/List");
  ...
  getProfileDetails(token, cb) {
    fetch(API_URL + GetProfile, {
      ...
      .then((resp) => resp.json())
      .then((data) => {
        console.log('getProfileCall-------------', data)
        cb({
          data,
        });
      })
