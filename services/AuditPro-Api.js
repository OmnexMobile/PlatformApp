...
// console.log('get baseURL---->', sURL1)
  ...
  export default {
    ...
    login(email, password, fcmToken, deviceId, loginFlag, isSso, cb) {
      ...
      console.log('checkingparams',formData)
      if (isSso !== null && isSso !== undefined) {
        formData.append('fromSSO', true);
      }
  
      console.log('tret Login formdata', formData);
      console.log('tret sURL formdata', sURL + login);
    ...
    loginPs(email, password, fcmToken, deviceId, loginFlag, isSso, cb) {
      var formData = new FormData();
      formData.append('UserName', email);
      formData.append('Password', password);
      formData.append('RegisteredDeviceId', deviceId);
      ...
      console.log('tret Login formdata', formData);
      console.log('tret sURL formdata', sURLPS + login);
    ...
    globalLoginUser(email, password, fcmToken, deviceId, loginFlag, isSso, cb) {
      var formData = new FormData();
      formData.append('UserName', email);
      formData.append('Password', password);
      formData.append('RegisteredDeviceId', deviceId);
      ...
      console.log('tret Login formdata', formData);
      console.log('tret sURL formdata', globalURL + login);
      fetch(globalURL + login, {
        method: 'POST',
        headers: {
          'Content-Type': 'multipart/form-data',
        },
    ...
    globalRegisterDevice(deviceId, url, type, cb) {
      var today = new Date();
      var dd = today.getDate();
      var mm = today.getMonth() + 1; //January is 0!
      var yyyy = today.getFullYear();
      ...
      fetch(globalURL + RegisterDevice,  {
        method: 'POST',
        headers: {
          'Content-Type': 'multipart/form-data',
        },
    ...
    auditlistapi(
      ...
      console.log('trets filter auditlistapi formData', formData);
      console.log('trets filter auditlistapi token', token, sURL + auditList);
    ...
    async getAllNCDetails(CorrectiveId, CorrectiveOrder, token, cb) {
      var formData = new FormData();
      ...
      await fetch(sURL + GetAllNCDetails, {
        method: 'POST',
        headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: 'Bearer' + ' ' + token,
