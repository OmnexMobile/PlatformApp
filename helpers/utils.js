...
export async function requestNotificationPermission() {
    ...
    if (Platform.OS === 'ios') {
        try {
            // ✅ Always force register — skip the isRegistered check entirely
            await registerDeviceForRemoteMessages(messaging);
            ...
            // ✅ Wait for APNS token with a retry loop (can take a moment on real devices)
            let apnsToken = null;
            let retries = 0;
            ...
            if (!apnsToken) {
                console.warn('❌ APNS token still null after retries — cannot get FCM token on iOS');
                return false; // Don't proceed if APNS token never arrived
            }
