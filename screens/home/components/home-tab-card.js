          ...
          {detail.map((items, index) => (
            items.category?.length ? (
              <TouchableOpacity
                ...
                key={`${title}-${index}`}
                onPress={() => handleNavigation(title, items?.navStatus ?? items?.status, items?.category, items?.auditTitle, items?.routeName, undefined, undefined, items?.status)}
              >
                {!!items.images && (
                  <LinearGradient
