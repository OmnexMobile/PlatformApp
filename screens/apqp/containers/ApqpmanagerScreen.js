...
import styles1 from "../components/styles/ApqpDashboardHeaderStyle";
...
class ApqpPpapManagerScreen extends Component {
  ...
  componentDidMount() {
    Reactotron.log(this.props?.route?.params, "navigationparamsapqp");
    this.setting();
    this.getData()
      .then((res) => {
  ...
  componentWillMount() {
    ...
    if (this.props.navigation.state.params) {
      if (this.props.navigation.state.params.ActiveTab) {
        if (this.props.navigation.state.params.ActiveTab == "today") {
          this.setState({ activeTab: 3 });
        }
      }
      if (this.props.navigation.state.params.isPendingTask == 1) {
      ...
      this.setState(
        {
          ...
          activeTab: this.props.navigation.state.params.activeTab
            ? this.props.navigation.state.params.activeTab
            : 0,
          loader: true,
        },
        () => {
...
getData = async () => {
  ...
  getCalendarList() {
    ...
    var TodayTask = 0;
    console.log(
      "@@@@@ APQPManagerScreen calendarapi", SiteId
    );
  ...
  getapqplistdata(from) {
    ...
    // console.log("id", this.state.selectedIndex, from);
  ...
  getTodaysTask(res) {
    ...
    NetInfo.fetch().then((netStatus) => {
      if (netStatus.isConnected) {
        console.log(
          "@@@@@ APQPManagerScreen1 calendarapi", SiteId
        );
        auth.calendarapi(UserID, Siteid, Token, TodayTask, (res, data) => {
          if (data.data.Message == "Success") {
  ...
  handleViewIndexChange = (index) => {
    ...
    this.setState(
      ...
      () => {
        this.props.navigation.navigate("ApqpProjectScreen");
      ...
      >
        <View style={styles1.wrapperFoot}>
          <View style={{ flexDirection: "row", padding: 10 }}>
            ...
            <TouchableOpacity
              onPress={() => this.props.navigation.navigate("Calenderview")}
              style={styles1.footerMenuItem}
            >
        ...
        >
          <View style={styles1.wrapperFoot}>
            <View style={{ flexDirection: "row", padding: 10 }}>
              <TouchableOpacity
                onPress={() =>
                  this.props.navigation.push("ApqpPpapManagerScreen", {
                    // ActiveTab: "today",
                    filterId: 2,
                    title: strings.projects,
                    todayn: 2,
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    To be Completed {`(${this.state.apqpTobecompleted})`}
                  </Text>
                </View>
              </TouchableWithoutFeedback>
              ...
              >
                ...
                >
                  ...
                  >
                    Pending {`(${this.state.apqpPending})`}
                  </Text>
                </View>
              </TouchableWithoutFeedback>
            </View>
