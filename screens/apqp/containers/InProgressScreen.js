...
import {
  ScrollView, Text, ImageBackground, Image, Dimensions,
  Keyboard, View, TextInput, TouchableOpacity, Button, FlatList
} from 'react-native'
...
class InProgressScreen extends Component {
      ...
      componentDidMount() {
        this.getData().then((res) =>{
          ...
          this.setState({isLoading: true}, () => {
            this.getapqpInprogressListdata() 
          })          
        }).catch((e)=>{
  ...
  getapqpInprogressListdata() {
    ...
    console.log('calling')
    auth.getapqpInprogressListdata(UserID, SiteID, Index, MaxRow, ListType, Token,FilterValue,FilterColumn,OrderBy,Sorttype,StartDate,EndDate, (res, data) => {
      ...
      if (data.data.Message == 'Success') {
        ...
        this.setState({
          apqpInprogressListdata: getList,
          completecount: data.data.Data.length,
          // apqpList:data.data.Data,
          isLoading: false,
          isRefreshing: false
        }, () => {
          console.log('setting up the list...', this.state.apqpInprogressListdata)
  ...
  //       //loader: true
  //     }, () => {
  //       this.props.navigation.navigate('ApqpmanagerScreen', {
  ...
  render() {
    ...
    const Filterdata = [
      ...
      const datas = this.state.apqpInprogressListdata
  // const segmentedTabs = ['To be Completed', 'New', 'Pending']
        ...
        return (
            ...
            <View style={styles.apqpTextView}>
                <ImageBackground
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        <View style={styles.backLogo}>
                            <TouchableOpacity style={{ width: '100%', justifyContent: 'center', alignItems: 'center' }} onPress={() => this.props.navigation.goBack()}>
                ...
                {this.state.searchFlag ?
                  ...
                  :
                  <Text style={styles.apqpTextStyle}>InProgress</Text>
                }
              </View>
              <TouchableOpacity onPress={() => this.onClearSearch()} style={styles.calendarDiv}>
                {this.state.searchFlag ?
                  <Icon style={{ top: 5 }} name="times-circle" size={28} color="white" /> :
                  <Icon style={{ top: 5 }} name="search" size={28} color="white" />
                }
              </TouchableOpacity>
                        <View style={styles.headerTextDiv}>
                            <Text style={styles.apqpTextStyle}>InProgress</Text>
                        </View>
                        <TouchableOpacity style={styles.calendarDiv}>
            ...
            <View style={{flexDirection:'column',width:'100%',height:'80%',top:80,position:'absolute'}}>
                      ...
                      <TouchableOpacity
                        ...
                        style={
                          item.id == this.state.activeFilter ?
                            [styles.buttonTouchableStyle, { backgroundColor: this.state.activeFilterColor }] :
                            [styles.buttonTouchableStyle, { backgroundColor: 'white' }]
                        }>
                        <Text>{item.name}</Text>
                      </TouchableOpacity>
                    </View>
          ...
          <View>
            ...
            <Text style={{paddingLeft:5}}>End date :- {this.state.EndDate}</Text>
            <TouchableOpacity onPress={()=>this.setState({
              ...
              <Icon style={{paddingLeft:10}} name={'times-circle'} size={18} color="grey"/>
            </TouchableOpacity>
            </View> :null
            }
          </View>:null
                ...
                <FlatList
                  ...
                  renderItem={({ item }) =>
                    <TouchableOpacity style={styles.flatListWholeView}> 
                      <View style={styles.flatListInsideView}>
                        <Text style={styles.listText}>Action :</Text>
                        <Text style={styles.actionTypeTextStylecb} numberOfLines={1}>{item.Actions}</Text>
                        {(item.Modules == "APQP") ?
                          <Image source={ Images.apqpModuleIcon } style={styles.apqpTypeIcon} /> :
                          null
                        }
                        {(item.Modules == "Risk") ?
                          <Image source={ Images.riskModuleIcon } style={styles.riskTypeIcon} /> :
                          null
                        }
                        {(item.Modules == "Meeting") ?
                          <Image source={ Images.meetingModuleIcon } style={styles.meetingTypeIcon} /> :
                          null
                        }
                      </View>
                      <View style={styles.flatListInsideView}>
                        <Text style={styles.listText}>Action Type :</Text>
                        <Text style={styles.actionTypeTextStyle} numberOfLines={1}>{item.ActionType}</Text>
                      </View>
                      <View style={styles.flatListInsideView}>
                        <Text style={styles.listText}>Site :</Text>
                        <Text style={styles.actionTypeTextStyle} numberOfLines={1}>{item.site}</Text>
                      </View>
                        <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                        <View style={styles.flatListInsideView}>
                        <Text style={styles.listText}>Period :</Text>
                        <Text style={[styles.dateTextStyle,{color:'#4C8048'}]} numberOfLines={1}>{this.changeDateFormatCard(item.StartDate)} - {this.changeDateFormatCard(item.DueDate)}</Text>
                      </View>
                      <View style={[styles.flatListInsideView,{flexDirection:'column',bottom:25}]}>
                      <Text style={
                        item.DueByDays > 0 ?
                        [styles.actionTypeTextStyle,{fontSize:22,color:'green'}]:[styles.actionTypeTextStyle,{fontSize:22,color:'red'}]
                        } numberOfLines={1}>{item.DueByDays}</Text>
          ...
          onBackdropPress={() => this.setState({ isDateVisible: false })}>
            <View style={styles.calendarDiv2}>
              <View style={styles.header}>
                <Text style={{fontSize:20,color:'#61BAD0'}}>Select Date Range</Text>
              </View>
                <CalendarPicker
                  ...
                  height={height(70)}
                 />
              <TouchableOpacity onPress={()=>{this.setState({ isDateVisible : false })}} style={styles.footer}>
...
export default connect(mapStateToProps, mapDispatchToProps)(InProgressScreen)
