import {StyleSheet, Dimensions, Platform} from 'react-native';
import {width, height} from 'react-native-dimension';
import Fonts from '../Themes/Fonts';

let Window = Dimensions.get('window');
const Width = Dimensions.get('window').width;
const Height = Dimensions.get('window').height;

export default StyleSheet.create({
  mainContainer: {
    flex: 1,
    width: '100%',
    backgroundColor:'#F5F7FA',
  },
  checkpointContent: {
    flex: 1,
    paddingBottom: 110,
  },
  backlogo: {
    flexDirection: 'row',
    backgroundColor: 'transparent',
    marginLeft: 10,
    height: 65,
    justifyContent: 'center',
    alignItems: 'center',
  },
  headerDiv: {
    width: '15%',
    height: 65,
    flexDirection: 'row',
    justifyContent: 'flex-end',
    alignItems: 'center',
  },
  slide: {
    width: '100%',
    height: '30%',
    backgroundColor: 'red',
    justifyContent: 'center',
    alignItems: 'center',
    marginTop: 20,
  },
  header: {
    width: '100%',
    // zIndex: 3000,
    flexDirection: 'row',
    //backgroundColor: 'white',
    padding: 5,
    alignContent: 'center',
    justifyContent: 'space-between',
    height: 65,
    elevation: 4,
    shadowOffset: {width: 2, height: 10},
    shadowColor: 'lightgrey',
    shadowOpacity: 0.5,
    shadowRadius: 4,
  },
  heading: {
    flexDirection: 'column',
    justifyContent: 'center',
    alignItems: 'center',
    width: '75%',
    height: 65,
    paddingHorizontal: width(2),
  },
  headingText: {
    fontSize: Math.min(Fonts.size.mediump, width(6)),
    color: 'black',
    textAlign: 'center',
    fontFamily: Fonts.type.bold,
    fontWeight: 'bold',
    width: '100%',
  },
  headerStatText: {
    fontSize: 14,
    color: '#fff',
    textAlign: 'center',
    fontFamily: 'OpenSans-Regular',
    marginTop: 2,
  },
  headerDiv: {
    width: width(15),
    height: 65,
    flexDirection: 'row',
    justifyContent: 'center',
    alignItems: 'center',
  },
  statistics: {
    width: '100%',
    backgroundColor: 'white',
    flexDirection: 'row',
    borderBottomWidth: 1,
    borderBottomColor: '#E3E8F0',
    paddingVertical: 4,
  },
  statCard1: {
    flex: 1,
    backgroundColor: 'white',
    flexDirection: 'column',
    justifyContent: 'center',
    alignItems: 'center',
    paddingVertical: 2,
  },
  statCard2: {
    width: '33.3%',
    flex: 1,
    height: '100%',
    backgroundColor: 'white',
    flexDirection: 'column',
    justifyContent: 'center',
    alignItems: 'center',
    borderLeftWidth: 0.8,
    borderLeftColor: 'lightgray',
  },
  statCard3: {
    flex: 1,
    backgroundColor: 'white',
    flexDirection: 'column',
    justifyContent: 'center',
    alignItems: 'center',
    paddingVertical: 2,
    borderLeftWidth: 0.8,
    borderLeftColor: 'lightgray',
  },
  cart: {
    width: '78%',
    paddingVertical: 5,
    marginTop: 10,
    // height: '90%',
    backgroundColor: 'white',
    shadowColor: 'grey',
    shadowOffset: {height: 0, width: 0},
    shadowOpacity: 0.4,
    shadowRadius: 5,
    elevation: 2,
    borderRadius: 20,
    // bottom: 20,
    marginBottom: 10,
    marginLeft: 1,
    marginRight: 20,
    // flexDirection:'row'
  },
  body: {
    flex: 1,
    width: '100%',
    flexDirection: 'column',
    marginTop: 0,
  },
  checkpointBody: {
    flex: 1,
    width: '100%',
    paddingBottom: 132,
  },
  bodyColumn: {
    flexDirection: 'column',
  },
  bodyRightRailLayout: {
    flexDirection: 'row',
    alignItems: 'flex-start',
  },
  carouselBottomWrapper: {
    flex: 1,
    width: '100%',
  },
  checkpointDeckViewport: {
    position: 'relative',
    overflow: 'visible',
  },
  carouselContainer: {
    flex: 1,
    width: '100%',
  },
  checkpointDeckActiveScroll: {
    zIndex: 3,
    elevation: 3,
    overflow: 'visible',
  },
  carouselSlide: {
    paddingHorizontal: 8,
  },
  carouselStackContainer: {
    overflow: 'visible',
  },
  carouselStackContent: {
    overflow: 'visible',
  },
  carouselStackSlide: {
    overflow: 'visible',
  },
  cartBottomLayout: {
    width: '100%',
    marginLeft: 0,
    marginRight: 0,
    alignSelf: 'stretch',
  },
  carouselStackCard: {
    width: '100%',
    marginLeft: 0,
    marginRight: 0,
    paddingBottom: 12,
    borderWidth: 1,
    borderColor: '#E8ECF2',
    borderRadius: 16,
    overflow: 'hidden',
    backgroundColor: '#FFFFFF',
  },
  checkpointDeckActiveCard: {
    alignSelf: 'flex-start',
    marginLeft: 12,
    zIndex: 3,
    elevation: 3,
  },
  checkpointPreviewCard: {
    position: 'absolute',
    top: 12,
    paddingTop: 16,
    borderWidth: 1,
    borderColor: '#DDE5F0',
    borderRadius: 16,
    backgroundColor: '#FFFFFF',
    shadowColor: '#8B98AD',
    shadowOffset: {width: 0, height: 8},
    shadowOpacity: 0.14,
    shadowRadius: 12,
    elevation: 7,
    zIndex: 7,
    overflow: 'hidden',
  },
  checkpointPreviewCardPrevious: {
    left: 0,
  },
  checkpointPreviewCardNext: {
    right: 0,
  },
  checkpointPreviewHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    paddingHorizontal: 18,
    paddingBottom: 14,
  },
  checkpointPreviewSerial: {
    flex: 1,
    color: '#8A909B',
    fontSize: Fonts.size.mediump,
    fontFamily: 'OpenSans-Regular',
    textAlign: 'center',
    paddingHorizontal: 10,
  },
  checkpointPreviewTitle: {
    color: '#1A1A1A',
    fontSize: Fonts.size.mediump,
    lineHeight: 24,
    fontFamily: 'OpenSans-SemiBold',
    paddingHorizontal: 18,
    paddingTop: 18,
    minHeight: 138,
  },
  checkpointPreviewDivider: {
    width: '100%',
    height: 1,
    backgroundColor: '#E9EDF4',
  },
  checkpointPreviewAttachmentButton: {
    height: 52,
    minWidth: 210,
    maxWidth: '72%',
    borderRadius: 14,
    borderWidth: 1,
    borderColor: '#D9DEE8',
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    alignSelf: 'flex-end',
    marginRight: 18,
    marginBottom: 18,
    paddingHorizontal: 14,
  },
  checkpointPreviewAttachmentText: {
    color: '#123C95',
    fontSize: Fonts.size.medium,
    fontFamily: 'OpenSans-Regular',
    marginLeft: 10,
  },
  checkpointPreviewField: {
    paddingHorizontal: 18,
    paddingVertical: 14,
  },
  checkpointPreviewFieldLabel: {
    color: '#333333',
    fontSize: Fonts.size.medium,
    fontFamily: 'OpenSans-Regular',
  },
  checkpointPreviewFieldValue: {
    color: '#1A1A1A',
    fontSize: Fonts.size.medium,
    fontFamily: 'OpenSans-Regular',
    paddingTop: 4,
    paddingBottom: 10,
    borderBottomWidth: 1,
    borderBottomColor: '#B8B8B8',
  },
  checkpointPreviewRemarkBox: {
    height: 150,
    borderRadius: 14,
    borderWidth: 1,
    borderColor: '#D9DEE8',
    marginTop: 12,
    paddingHorizontal: 14,
    paddingVertical: 12,
  },
  checkpointPreviewRemarkPlaceholder: {
    color: '#D9214B',
    fontSize: Fonts.size.medium,
    fontFamily: 'OpenSans-SemiBold',
  },
  checkpointDeckSideHitLayer: {
    position: 'absolute',
    top: 0,
    left: 0,
    right: 0,
    bottom: 0,
    zIndex: 8,
    elevation: 8,
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
  checkpointDeckSideHit: {
    width: 36,
    height: '100%',
    justifyContent: 'center',
    alignItems: 'center',
  },
  checkpointDeckSideHitLeft: {
    paddingLeft: 2,
  },
  checkpointDeckSideHitRight: {
    paddingRight: 2,
  },
  serialStripWrapper: {
    position: 'absolute',
    left: 0,
    right: 0,
    bottom: 74,
    zIndex: 2999,
    elevation: 11,
    width: '100%',
    paddingHorizontal: 8,
    paddingBottom: 8,
    backgroundColor: '#FFFFFF',
    borderTopWidth: 1,
    borderTopColor: '#E8ECF2',
  },
  serialDockContainer: {
    position: 'absolute',
    left: 0,
    right: 0,
    bottom: 74,
    backgroundColor: '#FFFFFF',
    borderTopWidth: 1,
    borderTopColor: '#E3E8F0',
    paddingHorizontal: 12,
    paddingVertical: 8,
    zIndex: 2999,
    elevation: 12,
    flexDirection: 'row',
    alignItems: 'center',
    minHeight: 52,
  },
  serialDockToggle: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#123C95',
    paddingHorizontal: 14,
    paddingVertical: 10,
    borderRadius: 22,
    minHeight: 44,
  },
  serialDockToggleText: {
    color: '#FFFFFF',
    fontSize: Fonts.size.medium,
    fontFamily: 'OpenSans-SemiBold',
    marginRight: 6,
  },
  serialDockList: {
    flex: 1,
    marginLeft: 10,
    maxHeight: 44,
  },
  serialFloatingPill: {
    width: 112,
    height: 44,
    borderRadius: 22,
    overflow: 'hidden',
    shadowColor: '#6A6A6A',
    shadowOffset: {width: 0, height: 8},
    shadowOpacity: 0.22,
    shadowRadius: 12,
    elevation: 8,
  },
  serialFloatingPillGradient: {
    width: '100%',
    height: '100%',
    borderRadius: 22,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    paddingHorizontal: 10,
  },
  serialFloatingPillText: {
    color: '#FFFFFF',
    fontSize: 15,
    fontFamily: 'OpenSans-Bold',
    marginRight: 5,
  },
  serialFloatingPanel: {
    flex: 1,
    height: 56,
    marginLeft: 10,
  },
  serialFloatingContainer: {
    position: 'absolute',
    left: 0,
    right: 0,
    bottom: 74,
    zIndex: 3001,
    elevation: 13,
    minHeight: 72,
    paddingHorizontal: 24,
    paddingVertical: 8,
    backgroundColor: '#FFFFFF',
    borderTopWidth: 1,
    borderTopColor: '#E3E8F0',
    flexDirection: 'row',
    alignItems: 'center',
  },
  serialFloatingContainerCollapsed: {
    position: 'absolute',
    left: 24,
    bottom: 86,
    zIndex: 3001,
    elevation: 13,
    backgroundColor: 'transparent',
    width: 112,
    height: 44,
    flexDirection: 'row',
    alignItems: 'center',
  },
  serialFloatingListContent: {
    alignItems: 'center',
    paddingRight: 8,
  },
  serialFloatingNumber: {
    minWidth: 58,
    height: 52,
    borderRadius: 18,
    borderWidth: 1,
    borderColor: '#D9DEE8',
    backgroundColor: '#FFFFFF',
    alignItems: 'center',
    justifyContent: 'center',
    marginHorizontal: 5,
  },
  serialFloatingNumberActive: {
    backgroundColor: '#123C95',
    borderColor: '#123C95',
  },
  serialFloatingNumberText: {
    color: '#10224C',
    fontSize: 18,
    fontFamily: 'OpenSans-Bold',
  },
  serialRailWrapper: {
    paddingLeft: 8,
    paddingRight: 4,
    paddingBottom: 8,
    alignItems: 'center',
    height: '100%',
    justifyContent: 'flex-end',
  },
  serialStripContent: {
    alignItems: 'center',
    paddingRight: 8,
  },
  serialRailContent: {
    paddingTop: 8,
    paddingBottom: 24,
    alignItems: 'center',
  },
  serialRailExpandedWrap: {
    flex: 1,
    width: '100%',
    alignItems: 'center',
    justifyContent: 'space-between',
  },
  serialRailList: {
    width: '100%',
    flexGrow: 0,
  },
  serialRailToggle: {
    minHeight: 42,
    width: '100%',
    borderRadius: 18,
    borderWidth: 1,
    borderColor: '#D9DEE8',
    backgroundColor: '#FFFFFF',
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    paddingHorizontal: 10,
    marginTop: 8,
    marginBottom: 12,
    shadowColor: '#AAB4C8',
    shadowOffset: {width: 0, height: 4},
    shadowOpacity: 0.12,
    shadowRadius: 8,
    elevation: 3,
  },
  serialRailToggleGradientShell: {
    borderWidth: 0,
    backgroundColor: 'transparent',
    overflow: 'hidden',
  },
  serialRailToggleGradient: {
    minHeight: 42,
    // width: '100%',
    borderRadius: 18,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    paddingHorizontal: 10,
  },
  serialRailToggleBottom: {
    marginTop: 0,
    marginBottom: 8,
  },
  serialRailToggleText: {
    fontSize: 12,
    color: '#1099BF',
    fontFamily: 'OpenSans-Bold',
    marginRight: 4,
  },
  serialRailToggleTextLight: {
    color: '#FFFFFF',
  },
  bottomSerialBtn: {
    minWidth: 84,
    paddingHorizontal: 12,
    height: 54,
    borderRadius: 12,
    borderWidth: 0.8,
    marginRight: 8,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: '#FFFFFF',
    overflow: 'hidden',
  },
  bottomSerialGradient: {
    width: '100%',
    height: '100%',
    borderRadius: 12,
    alignItems: 'center',
    justifyContent: 'center',
    paddingHorizontal: 12,
  },
  bottomSerialText: {
    fontSize: 16,
    textAlign: 'center',
    fontFamily: 'OpenSans-Regular',
  },
  serialRailButton: {
    borderRadius: 22,
    borderWidth: 1,
    marginBottom: 12,
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: '#FFFFFF',
    overflow: 'hidden',
    shadowColor: '#AAB4C8',
    shadowOffset: {width: 0, height: 6},
    shadowOpacity: 0.18,
    shadowRadius: 12,
    elevation: 4,
  },
  serialRailButtonGradient: {
    width: '100%',
    height: '100%',
    borderRadius: 22,
    alignItems: 'center',
    justifyContent: 'center',
  },
  serialRailText: {
    fontSize: 18,
    textAlign: 'center',
    fontFamily: 'OpenSans-Bold',
  },
  serialRailAccent: {
    width: 30,
    height: 6,
    borderRadius: 4,
    marginTop: 12,
  },
  bottomMandatoryIcon: {
    marginLeft: 6,
    bottom: 10,
  },
  leftBtn: {
    width: '95%',
    padding: 10,
    borderColor: 'grey',
    borderWidth: 0.4,
    marginLeft: 2,
    paddingVertical: 15,
    flexDirection: 'row',
    // bottom:5
  },
  bottomBtnView: {
    width: '100%',
    marginVertical: 10,
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
  backBtn: {
    flex: 0.5,
    backgroundColor: '#00BAC8',
    marginHorizontal: 5,
    paddingVertical: 12,
    borderRadius: 10,
    alignItems: 'center',
  },
  backBtnDisabled: {
    backgroundColor: 'lightgrey',
  },
  nextBtn: {
    flex: 0.5,
                backgroundColor: '#00BAC8',
                marginHorizontal: 5,
                paddingVertical: 12,
                borderRadius: 10,
                alignItems: 'center',
  },
  backBtnText: {
    fontSize: 16,
    color: 'white',
    fontFamily: 'OpenSans-Regular',
  },
  buttonDisabledText: {
    color: 'grey',
  },
  quesText: {
    fontSize: Fonts.size.mediump,
    width: '100%',
    color: 'black',
    fontFamily: 'OpenSans-Regular',
    padding: 5,
    marginTop: 5,
    // backgroundColor:'yellow',
  },
  boxsecRadio: {
    width: '100%',
    backgroundColor: 'transparent',
    flexDirection: 'row',
    paddingHorizontal: 12,
    justifyContent: 'flex-start',
    flexWrap: 'wrap',
    paddingTop: 6,
    paddingBottom: 4,
    alignItems: 'center',
    paddingRight: 8,
  },
  ncofiRow: {
    width: '100%',
    paddingHorizontal: 12,
    paddingVertical: 8,
    alignItems: 'flex-end',
  },
  ncofiButton: {
    paddingHorizontal: 18,
    paddingVertical: 10,
    backgroundColor: '#123C95',
    borderRadius: 22,
    minHeight: 44,
    justifyContent: 'center',
    alignItems: 'center',
    alignSelf: 'flex-end',
  },
  ncofiLabel: {
    color: '#FFFFFF',
    fontSize: Fonts.size.medium,
    fontFamily: 'OpenSans-SemiBold',
  },
  ncofi: {
    paddingHorizontal: 18,
    paddingVertical: 9,
    backgroundColor: '#123C95',
    borderRadius: 22,
    minHeight: 40,
    justifyContent: 'center',
    alignItems: 'center',
    alignSelf: 'flex-end',
    marginRight: 12,
  },
  radioLabelStyle: {
    color: '#1A1A1A',
    fontSize: Fonts.size.medium,
    fontFamily: 'OpenSans-SemiBold',
    paddingRight: 12,
    marginRight: 4,
    marginBottom: 2,
  },
  boxsecImageDisplay: {
    width: '50%',
    height: null,
    backgroundColor: 'white',
    /* borderBottomColor: '#808080',
    borderBottomWidth: 0.5, */
    flexDirection: 'column',
    paddingLeft: 10,
    paddingTop: 10,
    paddingRight: 10,
  },
  boxsecVideoDisplay: {
    //width: '50%',
    height: null,
    backgroundColor: 'white',
    /* borderBottomColor: '#808080',
    borderBottomWidth: 0.5, */
    flexDirection: 'column',
    paddingLeft: 10,
    paddingTop: 10,
    paddingRight: 10,
  },
  rightHeader: {
    flexDirection: 'row',
    justifyContent: 'flex-end',
    alignItems: 'center',
    //right: 10,
    //height: 80,
    paddingRight: 10,
  },
  boxsec1: {
    width: '100%',
    backgroundColor: 'white',
    flexDirection: 'column',
    paddingHorizontal: 12,
    paddingTop: 4,
  },
  checkpointContentColumn: {
    width: '100%',
    paddingHorizontal: 0,
  },
  boxsecNone: {
    // width:'98%',
    // height: null,
    // backgroundColor:'yellow',
    // borderBottomColor: 'lightgrey',
    // borderBottomWidth: 0.5,
    // flexDirection: 'column',
    // paddingLeft: 10
    display: 'none',
  },
  checkPointsTextInputLabel: {
    fontSize: Fonts.size.mediump,
    paddingTop: 0,
    marginTop: 0,
    fontFamily: 'OpenSans-Regular',
    color: '#000',
  },
  checkPointsTextInput: {
    fontSize: Fonts.size.mediump,
    paddingTop: 2,
    height: 50,
    fontFamily: 'OpenSans-Regular',
    color: '#000',
    // borderBottomColor: 'lightgrey',
    // borderTopWidth: 0.5,
  },
  attachIcon: {
    top: 3,
    position: 'absolute',
    right: 5,
    height: 40,
  },
  scoreBox: {
    position: 'absolute',
    //top: height(2), -- changes done 16/12/2022
    left: 50,
  },
  scoreText: {
    top: height(0),
    left: 0, //-- changes done 16/12/2022
    width: width(75),
  },
  LPAsec1: {
    //width:'100%',
    //height:'40%',
    // backgroundColor:'white'
    backgroundColor: 'transparent',
    // borderBottomColor: 'lightgrey',
    // borderTopWidth: 0.5,
  },
  LPAsec1Label: {
    //width:'100%',
    //height:'40%',
    // backgroundColor:'white'
    backgroundColor: 'transparent',
    // borderBottomColor: 'lightgrey',
    // borderTopWidth: 0.5,
    paddingTop: 0,
    marginTop: 0,
  },
  LPAsec2: {
    //width:'100%',
    //height:'60%',
    // backgroundColor:'grey'
    backgroundColor: 'white',
    borderBottomColor: 'lightgrey',
    borderTopWidth: 0.5,
  },
  boxsecRemark: {
    width: Platform.OS === 'ios' ? '85%' : '90%',
    height: null,
    backgroundColor: '#ffffff',
    /* borderBottomColor: '#808080',
    borderBottomWidth: 0.5, */
    flexDirection: 'row',
    paddingLeft: 5,
    // marginLeft: 5,
    justifyContent: 'space-between',
  },
  modalOuterBox: {
    backgroundColor: 'rgba(0,0,0,0.8)',
    width: '100%',
    height: '100%',
    justifyContent: 'center',
    alignItems: 'center',
    margin: 0,
    top: 0,
    left: 0,
  },
  ncModal: {
    //flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    width: '90%',
    //height: '40%',
    backgroundColor: 'white',
    borderRadius: 10,
    margin: 10,
    padding: 10,
    elevation: 8,
    borderColor: 'lightgrey',
    borderWidth: 0.5,
  },
  ModalBox: {
    width: width(90),
    height: height(50),
    backgroundColor: 'white',
    borderRadius: 10,
    flexDirection: 'column',
    top: height(40),
  },
  modalheader: {
    width: width(90),
    height: height(8),
    backgroundColor: 'transparent',
    top: 10,
    justifyContent: 'center',
    alignItems: 'center',
    borderBottomWidth: 1,
    borderBottomColor: 'white',
  },
  modalbody: {
    width: width(90),
    height: height(42),
    backgroundColor: 'white',
    padding: 20,
  },
  sectionTop: {
    backgroundColor: 'white',
    flexDirection: 'column',
    justifyContent: 'flex-start',
    alignItems: 'flex-start',
    borderBottomWidth: 0.5,
    borderBottomColor: '#C4C4C4',
    padding: 10,
  },
  modalheading: {
    flexDirection: 'row',
    justifyContent: 'center',
    backgroundColor: 'transparent',
    borderBottomColor: 'lightgrey',
    borderBottomWidth: 0.8,
    justifyContent: 'center',
    alignItems: 'center',
    padding: 20,
  },
  sectionContent: {
    flexDirection: 'row',
    justifyContent: 'flex-start',
    alignItems: 'center',
  },
  boxContent: {
    width: '100%',
    color: 'black',
    fontSize: Fonts.size.mediump,
    textAlign: 'center',
    fontFamily: 'OpenSans-Regular',
  },
  sectionTopCancel: {
    backgroundColor: 'white',
    flexDirection: 'column',
    justifyContent: 'flex-start',
    alignItems: 'flex-start',
    padding: 10,
  },
  boxContentClose: {
    width: '100%',
    color: '#000',
    textAlign: 'center',
    fontSize: Fonts.size.mediump,
    fontFamily: 'OpenSans-Regular',
  },
  sectionBtn: {
    backgroundColor: 'white',
    flexDirection: 'column',
    justifyContent: 'center',
    alignItems: 'center',
    borderBottomWidth: 0.5,
    borderBottomColor: '#C4C4C4',
    padding: 10,
  },
  boxContentCam: {
    //width: '100%',
    color: '#485B9E',
    fontSize: Fonts.size.mediump,
    // textAlign: 'center',
    // paddingLeft: 20,
    fontFamily: 'OpenSans-Regular',
  },
  modalavatar: {
    flex: 1,
    width: width(90),
    justifyContent: 'center',
    alignContent: 'center',
    paddingTop: 20,
    margin: 20,
  },
  modelImage: {
    width: '100%',
    height: '80%',
    resizeMode: 'contain',
    justifyContent: 'center',
    alignContent: 'center',
  },
  footer: {
    position: 'absolute',
    left: 0,
    right: 0,
    bottom: 0,
    flexDirection: 'row',
    justifyContent: 'flex-start',
    alignContent: 'center',
    width: '100%',
    backgroundColor: 'transparent',
    height: 74,
    zIndex: 3000,
    elevation: 12,
  },
  footerDiv: {
    flexDirection: 'row',
    justifyContent: 'flex-start',
    alignContent: 'center',
    width: '100%',
    height: '100%',
  },
  footerWhite: {
    backgroundColor: '#FFFFFF',
  },
  footerActionRow: {
    flexDirection: 'row',
    alignItems: 'center',
    width: '100%',
    height: '100%',
    paddingHorizontal: width(3),
  },
  footerActionButton: {
    flex: 1,
    height: '100%',
    justifyContent: 'center',
    alignItems: 'center',
  },
  footerActionButtonLeft: {
    marginRight: 8,
  },
  footerActionButtonRight: {
    marginLeft: 8,
  },
  footerGradientButton: {
    width: '100%',
    height: 46,
    borderRadius: 12,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    overflow: 'hidden',
  },
  footerActionText: {
    marginLeft: 10,
    fontSize: Fonts.size.mediump,
    fontFamily: 'OpenSans-SemiBold',
    color: 'white',
  },
  noRecordsFound: {
    width: '100%',
    textAlign: 'center',
    fontSize: Fonts.size.h5,
    color: 'grey',
    fontFamily: 'OpenSans-Regular',
  },
  noRecordsContainer: {
    marginTop: 55,
    minHeight: 280,
    justifyContent: 'center',
    alignItems: 'center',
  },
  noRecordsLottie: {
    width: Window.width * 0.44,
    height: Window.width * 0.44,
  },

  // Attachments
  attachmentFailedCard: {
    flexDirection: 'row',
    borderColor: 'darkgrey',
    paddingVertical: 10,
    margin: 2,
    borderWidth: 1,
    borderRadius: 5,
  },
  row: {
    flexDirection: 'row',
  },
  attachmentFilenameWrap: {
    width: width(65),
    marginTop: 5,
    alignContent: 'center',
    alignItems: 'center',
    alignSelf: 'center',
  },
  attachmentCard: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    paddingVertical: 10,
    margin: 2,
    borderColor: 'darkgrey',
    borderWidth: 1,
    height: '90%',
    borderRadius: 5,
  },
  attachmentInnerPadding: {
    paddingVertical: 10,
    margin: 2,
  },
  attachmentImage: {
    width: width(65),
    height: 200,
    resizeMode: 'cover',
    marginRight: 15,
  },
  hiddenView: {
    display: 'none',
  },
  attachmentDocCard: {
    flexDirection: 'row',
    borderColor: 'darkgrey',
    borderWidth: 1,
    borderRadius: 5,
    height: '90%',
    paddingVertical: 10,
    margin: 2,
  },

  // Loader
  loaderContainer: {
    backgroundColor: 'white',
    width: '100%',
    minHeight: 100,
    alignItems: 'center',
    justifyContent: 'center',
  },

  // Download icon
  downloadIndicator: {
    paddingTop: 70,
    height: 200,
    zIndex: 1,
    flex: 1,
    justifyContent: 'center',
    alignSelf: 'center',
  },
  downloadFileNameWrap: {
    width: width(65),
    marginTop: 5,
    alignContent: 'center',
    alignItems: 'center',
    alignSelf: 'center',
  },
  downloadOverlayIcon: {
    position: 'absolute',
    paddingTop: 85,
    zIndex: 1,
    flex: 1,
    justifyContent: 'center',
    alignSelf: 'center',
  },
  attachmentIconLarge: {
    paddingTop: 70,
    height: 200,
    flex: 1,
    justifyContent: 'center',
    alignSelf: 'center',
  },
  attachmentFailedLabel: {
    zIndex: 1,
    position: 'absolute',
    paddingTop: 135,
    flex: 1,
    color: 'red',
    justifyContent: 'center',
    alignSelf: 'center',
  },
  paperclipIcon: {
    bottom: 2,
  },
  attachmentStarIcon: {
    bottom: 20,
    right: 10,
  },
  sliderColumn: {
    flexDirection: 'column',
  },
  sliderControl: {
    width: '80%',
  },
  sliderThumbStyle: {
    elevation: 5,
    backgroundColor: 'white',
    borderColor: 'black',
    borderWidth: 0.5,
  },
  sliderValueRow: {
    padding: 6,
    bottom: 12,
    flexDirection: 'row',
    justifyContent: 'space-between',
    width: '80%',
  },
  sliderValueText: {
    fontFamily: 'OpenSans-Regular',
  },
  sliderValueRowAlt: {
    padding: 5,
    bottom: 15,
    flexDirection: 'row',
    justifyContent: 'space-between',
    width: '80%',
  },
  scoreLabelText: {
    color: 'black',
  },
  circleIconWrapper: {
    marginHorizontal: 5,
  },
  dropdownLabel: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    width: '100%',
  },
  dropdownTextStyle: {
    numberOfLines: 2,
  },
  scoreTypesContainer: {
    padding: 15,
    flexDirection: 'column',
  },
  scoreTypesHidden: {
    padding: 15,
    flexDirection: 'column',
    backgroundColor: 'lightgrey',
    width: '80%',
    borderRadius: 10,
    display: 'none',
  },
  scoreLabel: {
    padding: 0,
    margin: 0,
    color: '#1A1A1A',
    width: '90%',
    fontSize: Fonts.size.mediump,
    fontFamily: 'OpenSans-SemiBold',
  },
  scoreLabelInvalid: {
    color: 'red',
  },
  scoreMaxText: {
    fontSize: Fonts.size.regular,
    fontFamily: 'OpenSans-Regular',
  },
  scoreSmallLabel: {
    padding: 0,
    margin: 0,
    color: '#A6A6A6',
    width: '90%',
    fontSize: Fonts.size.small,
    fontFamily: 'OpenSans-Regular',
  },
  fullWidth: {
    width: '100%',
  },
  rowCenter: {
    flexDirection: 'row',
    alignItems: 'center',
  },
  dropdownOffset: {
    top: 10,
    left: 0,
  },
  dropdownItemText: {
    fontFamily: 'OpenSans-Regular',
  },
  immediateDeleteButton: {
    marginLeft: 10,
    marginTop: 5,
    padding: 5,
  },
  dropdownAngleIcon: {
    marginLeft: 6,
    marginTop: 5,
  },
  bottomSpacer: {
    width: '100%',
    height: 16,
  },
  remarkStarIcon: {
    right: 10,
    top: 10,
  },
  sectionSpacer: {
    marginTop: 5,
  },
  dropdownContainer: {
    paddingTop: 5,
  },

  // Attachment loading
  attachmentLoadingRow: {
    flexDirection: 'row',
    paddingBottom: 10,
  },
  hourglassIcon: {
    padding: 5,
  },
  attachmentLoadingText: {
    color: '#A6A6A6',
    fontFamily: 'OpenSans-Regular',
    alignSelf: 'flex-start',
    padding: 5,
  },

  // Content loader
  contentLoaderContainer: {
    flexDirection: 'column',
    justifyContent: 'center',
    alignItems: 'center',
    width: '100%',
    height: '100%',
  },
  contentLoaderTitle: {
    fontSize: Fonts.size.regular,
    fontFamily: 'OpenSans-Regular',
  },
  contentLoaderSubtitle: {
    fontSize: Fonts.size.small,
    fontFamily: 'OpenSans-Regular',
  },

  // Action buttons
  actionBar: {
    flexDirection: 'row',
    justifyContent: 'space-evenly',
    paddingHorizontal: 5,
    paddingVertical: 5,
  },
  resetButton: {
    flex: 0.45,
    backgroundColor: '#00BAC8',
    marginHorizontal: 2,
    paddingVertical: 12,
    borderRadius: 10,
    alignItems: 'center',
  },
  saveButton: {
    flex: 0.45,
    backgroundColor: '#00BAC8',
    marginHorizontal: 2,
    paddingVertical: 12,
    borderRadius: 10,
    alignItems: 'center',
  },
  saveButtonDisabled: {
    opacity: 0.7,
  },
  savingOverlay: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: 'rgba(255, 255, 255, 0.72)',
    alignItems: 'center',
    justifyContent: 'center',
    zIndex: 5000,
    elevation: 20,
  },
  savingOverlayCard: {
    backgroundColor: '#FFFFFF',
    borderRadius: 16,
    paddingVertical: 24,
    paddingHorizontal: 32,
    alignItems: 'center',
    justifyContent: 'center',
    shadowColor: '#0F172A',
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.12,
    shadowRadius: 12,
    elevation: 8,
    minWidth: 160,
  },
  savingOverlayText: {
    marginTop: 12,
    fontSize: 15,
    fontFamily: 'OpenSans-SemiBold',
    color: '#123C95',
  },
  actionButtonText: {
    textAlign: 'center',
    fontSize: 14,
    fontWeight: '600',
    color: '#fff',
  },

  listMarginTop: {
    marginTop: 55,
  },

  // Modal heading
  centerAlignedRow: {
    justifyContent: 'center',
    alignItems: 'center',
  },
  modalHeadingText: {
    color: 'black',
    fontSize: Fonts.size.regular,
    fontFamily: 'OpenSans-Regular',
  },

  // Transparent close buttons
  transparentCloseButton: {
    backgroundColor: 'transparent',
    height: 60,
    width: 80,
  },
  transparentCloseInner: {
    backgroundColor: 'transparent',
    top: 18,
  },
  closeIconOffset: {
    left: 8,
  },

  // Video modal
  videoContainer: {
    backgroundColor: 'black',
    flex: 1,
  },
  videoInner: {
    height: '80%',
  },
  videoPlayer: {
    position: 'absolute',
    top: 0,
    left: 0,
    right: 0,
    bottom: 0,
  },

  // Toast
  toastContainer: {
    backgroundColor: 'black',
    margin: 20,
  },
  toastText: {
    color: 'white',
  },

  // Generic helpers
  flexOne: {
    flex: 1,
  },
  iconOffsetTop: {
    marginTop: 10,
  },

  // Stats
  statLabel: {
    fontSize: Fonts.size.small,
    color: '#4A5568',
    fontFamily: 'OpenSans-Regular',
    textAlign: 'center',
  },
  statValue: {
    fontSize: Fonts.size.mediuml,
    color: '#123C95',
    fontFamily: 'OpenSans-Bold',
    marginTop: 2,
    textAlign: 'center',
  },
  questionTopActionsRow: {
    width: '100%',
    flexDirection: 'row',
    justifyContent: 'flex-end',
    paddingHorizontal: 12,
    paddingTop: 2,
    paddingBottom: 4,
  },
  questionTopActionsRight: {
    flexDirection: 'row',
    alignItems: 'center',
  },
  attachIconButton: {
    width: 42,
    height: 42,
    borderRadius: 21,
    borderWidth: 1,
    borderColor: '#C9D2DF',
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: '#FFFFFF',
    marginRight: 8,
  },
  attachIconRequired: {
    position: 'absolute',
    top: 2,
    right: 2,
  },
  ncofiCompactButton: {
    paddingHorizontal: 12,
    paddingVertical: 8,
    backgroundColor: '#123C95',
    borderRadius: 20,
    minHeight: 42,
    justifyContent: 'center',
    alignItems: 'center',
  },
  ncofiCompactLabel: {
    color: '#FFFFFF',
    fontSize: Fonts.size.small,
    fontFamily: 'OpenSans-SemiBold',
  },

  // Checkpoint list
  checkpointListWrapper: {
    flex: 1,
    height: '100%',
    marginTop: 5,
    bottom: 5,
  },
  leftBtnLabelWrapper: {
    width: '90%',
  },
  checkpointSerialText: {
    fontSize: 16,
    textAlign: 'center',
    fontFamily: 'OpenSans-Regular',
  },
  checkpointSerialTextActive: {
    color: 'white',
  },
  checkpointSerialTextInactive: {
    color: 'black',
  },
  requirementIconCol: {
    width: '10%',
    marginRight: 5,
  },
  requirementIconWrapper: {
    bottom: 15,
  },
  requirementIconWrapperSmall: {
    bottom: 5,
  },
  requirementIconWrapperIos: {
    marginLeft: 2,
  },

  carouselWrapper: {
    flex: 4,
    height: '100%',
    marginBottom: 10,
  },
  checkpointScroll: {
    flex: 1,
    width: '100%',
  },
  checkpointScrollItem: {
    width: '100%',
  },
  checkpointScrollContent: {
    paddingBottom: 220,
    flexGrow: 1,
  },
  checkpointFormSections: {
    width: '100%',
    flexDirection: 'column',
  },

  checkpointHeaderRow: {
    flexDirection: 'row',
    width: '90%',
  },
  checkpointHeaderCol: {
    width: '100%',
  },
  checkpointTitleRow: {
    flexDirection: 'row',
  },
  targetIconWrapper: {
    marginLeft: 5,
  },
  vetoIconWrapper: {
    justifyContent: 'flex-start',
    marginLeft: 10,
    marginTop: 5,
  },
  statusSpacer: {
    alignItems: 'flex-end',
    flexDirection: 'row',
    justifyContent: 'flex-end',
    height: 5,
    width: '100%',
    marginLeft: 40,
  },

  scoreRow: {
    flexDirection: 'row',
    width: '15%',
  },
  scoreLabel: {
    padding: 0,
    margin: 0,
    color: '#1A1A1A',
    width: '90%',
    fontSize: Fonts.size.mediump,
    fontFamily: 'OpenSans-SemiBold',
  },
  scoreValue: {
    paddingLeft: 10,
    fontFamily: 'OpenSans-Regular',
  },
  attachmentLabel: {
    paddingBottom: 10,
    margin: 0,
    color: '#A6A6A6',
    width: '90%',
    fontSize: Fonts.size.medium,
    fontFamily: 'OpenSans-Regular',
  },
  attachmentPreviewBlock: {
    width: '100%',
    marginTop: 6,
    marginBottom: 12,
  },
  attachmentPreviewContent: {
    alignItems: 'flex-start',
    paddingRight: 8,
  },
  attachmentGridContainer: {
    width: '100%',
    borderWidth: 1,
    borderColor: '#E3E8F0',
    borderRadius: 18,
    backgroundColor: '#FFFFFF',
    overflow: 'hidden',
  },
  attachmentGridHeader: {
    width: '100%',
    minHeight: 56,
    paddingHorizontal: 14,
    paddingVertical: 10,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  },
  attachmentGridTitle: {
    fontSize: Fonts.size.mediump,
    color: '#1A1A1A',
    fontFamily: 'OpenSans-SemiBold',
  },
  attachmentGridViewAllBtn: {
    flexDirection: 'row',
    alignItems: 'center',
  },
  attachmentGridViewAllText: {
    fontSize: 16,
    color: '#1E7CB8',
    fontFamily: 'OpenSans-Regular',
    marginRight: 2,
  },
  attachmentGridDivider: {
    width: '100%',
    height: 1,
    backgroundColor: '#E8EDF4',
  },
  attachmentGridList: {
    width: '100%',
    paddingHorizontal: 8,
    paddingTop: 10,
    paddingBottom: 4,
    flexDirection: 'row',
    flexWrap: 'wrap',
    justifyContent: 'space-between',
  },
  attachmentGridCard: {
    width: '49%',
    borderWidth: 1,
    borderColor: '#DDE4EE',
    borderRadius: 14,
    backgroundColor: '#FFFFFF',
    overflow: 'hidden',
    marginBottom: 10,
    position: 'relative',
  },
  attachmentGridCardPress: {
    width: '100%',
  },
  attachmentGridMediaWrap: {
    width: '100%',
    height: 106,
    backgroundColor: '#F7FAFD',
    alignItems: 'center',
    justifyContent: 'center',
  },
  attachmentGridMediaImage: {
    width: '100%',
    height: '100%',
    resizeMode: 'cover',
  },
  attachmentGridPlaceholder: {
    width: '100%',
    height: '100%',
    alignItems: 'center',
    justifyContent: 'center',
  },
  attachmentGridExtText: {
    marginTop: 4,
    color: '#7D8796',
    fontSize: 10,
    fontFamily: 'OpenSans-Regular',
  },
  attachmentGridBody: {
    width: '100%',
    paddingHorizontal: 10,
    paddingTop: 8,
    paddingBottom: 10,
  },
  attachmentGridName: {
    color: '#1A1A1A',
    fontSize: Fonts.size.medium,
    fontFamily: 'OpenSans-SemiBold',
  },
  attachmentGridMetaRow: {
    marginTop: 6,
    flexDirection: 'row',
    alignItems: 'center',
  },
  attachmentGridMetaText: {
    marginLeft: 8,
    color: '#4A5568',
    fontSize: Fonts.size.small,
    fontFamily: 'OpenSans-Regular',
  },
  attachmentGridDelete: {
    position: 'absolute',
    top: 8,
    right: 8,
    zIndex: 2,
    padding: 2,
  },
  attachmentListScroller: {
    width: '100%',
    borderWidth: 1,
    borderColor: '#E3E8F0',
    borderRadius: 16,
    backgroundColor: '#FFFFFF',
  },
  attachmentListContent: {
    width: '100%',
  },
  attachmentListRow: {
    minHeight: 104,
    borderBottomWidth: 1,
    borderBottomColor: '#E9EDF4',
    paddingHorizontal: 10,
    paddingVertical: 10,
    flexDirection: 'row',
    alignItems: 'flex-start',
  },
  attachmentListRowLast: {
    borderBottomWidth: 0,
  },
  attachmentListBody: {
    flex: 1,
    flexDirection: 'row',
    alignItems: 'center',
    marginRight: 8,
  },
  attachmentThumbWrap: {
    width: 92,
    height: 72,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: '#D8E0EC',
    backgroundColor: '#F6F8FC',
    overflow: 'hidden',
    alignItems: 'center',
    justifyContent: 'center',
  },
  attachmentThumbImage: {
    width: '100%',
    height: '100%',
    resizeMode: 'cover',
  },
  attachmentThumbPlaceholder: {
    width: '100%',
    height: '100%',
    alignItems: 'center',
    justifyContent: 'center',
  },
  attachmentThumbLabel: {
    marginTop: 4,
    color: '#7D8796',
    fontSize: 10,
    fontFamily: 'OpenSans-Regular',
  },
  attachmentListInfo: {
    flex: 1,
    paddingLeft: 12,
    justifyContent: 'center',
  },
  attachmentListName: {
    color: '#2A3042',
    fontSize: 18,
    fontFamily: 'OpenSans-Regular',
  },
  attachmentListMetaRow: {
    marginTop: 8,
    flexDirection: 'row',
    alignItems: 'center',
  },
  attachmentListMetaText: {
    marginLeft: 8,
    color: '#7F8793',
    fontSize: 14,
    fontFamily: 'OpenSans-Regular',
  },
  attachmentListDelete: {
    paddingHorizontal: 6,
    paddingVertical: 6,
    alignSelf: 'flex-start',
  },
  questionMetaRow: {
    width: '100%',
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    paddingHorizontal: 10,
    paddingTop: 16,
    paddingBottom: 10,
  },
  questionMetaNavButton: {
    width: 48,
    height: 40,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    flexShrink: 0,
  },
  questionMetaNavButtonRight: {
    justifyContent: 'center',
  },
  questionMetaNavButtonDisabled: {
    opacity: 0.45,
  },
  questionMetaNavText: {
    fontSize: 8,
    color: '#123C95',
    fontFamily: 'OpenSans-Regular',
    marginHorizontal: 6,
  },
  questionMetaNavTextDisabled: {
    color: '#9AA6B5',
  },
  questionMetaText: {
    flex: 1,
    textAlign: 'center',
    fontSize: Fonts.size.mediump,
    lineHeight: 26,
    color: '#1A1A1A',
    fontFamily: 'OpenSans-SemiBold',
    paddingHorizontal: 8,
  },
  questionMetaDivider: {
    width: '100%',
    height: 1,
    backgroundColor: '#E3E8F0',
    marginBottom: 10,
  },
  questionBlockRow: {
    flexDirection: 'row',
    width: '100%',
    paddingHorizontal: 12,
  },
  questionBlockTextCol: {
    flex: 1,
    minWidth: 0,
  },
  answerChoiceRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    width: '100%',
    paddingRight: 6,
    marginBottom: 6,
  },
  answerChoiceButton: {
    flex: 1,
    height: 58,
    borderRadius: 14,
    borderWidth: 1,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    marginRight: 8,
    paddingHorizontal: 10,
  },
  answerChoiceButtonBase: {
    shadowColor: '#93A2B9',
    shadowOffset: {height: 2, width: 0},
    shadowOpacity: 0.12,
    shadowRadius: 5,
    elevation: 1,
  },
  answerChoiceButtonInactive: {
    backgroundColor: '#FFFFFF',
    borderColor: '#D3DAE6',
  },
  answerChoiceButtonOkActive: {
    backgroundColor: '#60BE5A',
    borderColor: '#60BE5A',
  },
  answerChoiceButtonNotOkActive: {
    backgroundColor: '#F25858',
    borderColor: '#F25858',
  },
  answerChoiceButtonNaActive: {
    backgroundColor: '#EFF3F9',
    borderColor: '#D3DAE6',
  },
  answerChoiceIconCircle: {
    width: 34,
    height: 34,
    borderRadius: 17,
    borderWidth: 2,
    borderColor: '#C6CFDA',
    backgroundColor: '#FFFFFF',
    alignItems: 'center',
    justifyContent: 'center',
    marginRight: 10,
  },
  answerChoiceIconCircleActive: {
    borderColor: '#FFFFFF',
  },
  answerChoiceText: {
    fontSize: 18,
    fontFamily: 'OpenSans-Regular',
  },
  attachmentActionRow: {
    width: '100%',
    paddingHorizontal: 12,
    paddingTop: 8,
    paddingBottom: 4,
    alignItems: 'flex-end',
  },
  attachmentActionButton: {
    width: 46,
    height: 46,
    borderRadius: 23,
    borderWidth: 1.2,
    borderColor: '#C9D2DF',
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: '#FFFFFF',
    marginRight: 12,
  },
  attachmentActionIcon: {
    marginRight: 0,
  },
  attachmentActionLabel: {
    fontSize: Fonts.size.mediump,
    color: '#123C95',
    fontFamily: 'OpenSans-SemiBold',
  },
  attachmentActionRequired: {
    position: 'absolute',
    top: 5,
    right: 5,
  },
  scoreSectionCard: {
    borderTopWidth: 1,
    borderTopColor: '#E3E8F0',
    marginTop: 6,
    paddingTop: 8,
    backgroundColor: 'transparent',
  },
  remarkSectionCard: {
    width: '100%',
    marginTop: 12,
    paddingHorizontal: 12,
    paddingBottom: 16,
    borderTopWidth: 1,
    borderTopColor: '#E8ECF2',
    backgroundColor: 'transparent',
  },
  remarkLabelRow: {
    width: '100%',
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 6,
  },
  remarkLabelText: {
    padding: 0,
    margin: 0,
    color: '#1A1A1A',
    width: 'auto',
    fontSize: Fonts.size.mediump,
    fontFamily: 'OpenSans-SemiBold',
  },
  remarkLabelAsterisk: {
    marginLeft: 6,
    marginTop: -6,
  },
  remarkTextArea: {
    minHeight: 120,
    borderWidth: 1,
    borderColor: '#D1D8E4',
    borderRadius: 14,
    paddingHorizontal: 14,
    paddingVertical: 12,
    marginTop: 8,
    marginBottom: 12,
    color: '#1A1A1A',
    fontSize: Fonts.size.input,
    fontFamily: 'OpenSans-Regular',
    lineHeight: 24,
    backgroundColor: '#FFFFFF',
  },
  cardPagerRow: {
    marginTop: 0,
    marginBottom: 4,
    paddingTop: 0,
    borderTopWidth: 0,
  },
  cardPagerButton: {
    minHeight: 56,
    borderRadius: 16,
    justifyContent: 'center',
  },
  cardPagerButtonEnabled: {
    backgroundColor: '#00BAC8',
  },
  cardPagerButtonDisabled: {
    backgroundColor: '#D0D2D7',
  },
  cardPagerButtonText: {
    fontSize: 20,
    fontFamily: 'OpenSans-Regular',
  },
  cardPagerButtonTextEnabled: {
    color: '#FFFFFF',
  },
  cardPagerButtonTextDisabled: {
    color: '#7F8793',
  },
  cardPagerNextWrap: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
  },
});
