...
class AuditDashboardFooter extends Component {
  ...
  chooseAction(index) {
    ...
    if (index == 0) {
      this.props.navigation.navigate(ROUTES.PROFILE_SCREEN);
    }
    if (index == 1) {
      this.props.navigation.navigate(ROUTES.USER_PREFERENCE);
    }
    if (index == 2) {
          ...
          this.setState(
            {
             ...
             () => {
              // auth.registerDevice(
              auth.globalRegisterDevice(

                this.state.deviceId,
                // this.state.store_server_url,
                this.globalServerURL,
                type,
                async (res, data) => {
                  ...
                  console.log('this.globalServerURL', this.globalServerURL)
                  if (data.data) {
                    if (data?.data?.Success == true) {
                      console.log('ssoenableflag111::::::', this.state.sso_enabled_Flag);
                      if(this.state.sso_enabled_Flag == "true"){
                        console.log('this.state.sso_enabled_Flag', this.state.sso_enabled_Flag)
    ...
    // auth.globalLogin(
      ...
      async (res, data) => {
        ...
        if (data.data.Message == "Logout Successfully") {
          ...
          console.log('globalDeviceDetails--->Auditpro', this.globalServerURL)
          localStorage.storeData(LOCAL_STORAGE_VARIABLES.GLOBAL_SERVER_URL, this.globalServerURL)
          globalAuth.setServerUrl(this.globalServerURL);
          console.log('globalDeviceDetails--->Auditpro2', this.globalServerURL)
          localStorage.storeData(LOCAL_STORAGE_VARIABLES.Token, '');
          console.log('globalDeviceDetails--->Auditpro--end', this.globalServerURL)
          this.props.navigation.navigate(ROUTES.GLOBAL_LOGIN)
    ...
    return (
      <View style={styles.wrapper}>
        ...
        <View style={{flexDirection: 'row', padding: 10}}>
          ...
          <TouchableOpacity
            onPress={() => this.props.navigation.navigate(ROUTES.CALENDER_LIST)}
