...
class AllTabAuditList extends Component {
    ...
    return (
      <View style={styles.filterCont}>
        <TouchableOpacity
          ...
          onPress={() => this.props.navigation.navigate(ROUTES.FILTER_SCREEN)}>
  ...
  allAudits() {
    console.log('this.state.auditList---> all audits', this.state.auditList)
    return (
                ...
                <View
                  ...
                  <View style={styles.auditBoxContent}>
                    ...
                    <Text
                      numberOfLines={1}
                      style={[
                        {fontFamily: 'OpenSans-Regular', color: 'black'},
                      ]}>
                      {this.changeDateFormatCard(item.StartDate)} -{' '}
                      {this.changeDateFormatCard(item.EndDate)}
                    </Text>
            ...
            if (data.data.Message === 'Success') {
              if (data.data.Data && data.data.Data.length > 0) {
                ...
                _.forEach(auditList, function (Audit_res) {
                  ...
                  auditInfo['key'] =
                    Audit_res.ActualAuditId + '_' + (keyVal + 1);

                  // Set Audit Status
                  if (
                    Audit_res.AuditStatus == 3 &&
                    (Audit_res.CloseOutStatus == '7' ||
                      Audit_res.CloseOutStatus === '9')
                  ) {
        ...
        for (var j = 0; j < AllauditList.length; j++) {
          ...
          if (recent[i].ActualAuditId == AllauditList[j].ActualAuditId) {
            ...
            if (index != -1)
              ...
              total_arr.push({
                ...AllauditList[j],
                cStatus: AllauditRecordList[index].AuditRecordStatus,
              });
            flag = true;
          }
        }
