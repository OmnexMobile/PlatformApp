...
class CheckPointDemo extends Component {
    ...
    summarizeNcRecords = records => {
        ...
        records.forEach(record => {
            uploadedItems += Array.isArray(record?.Uploaded) ? record.Uploaded.length : 0;
            pendingItems += Array.isArray(record?.Pending) ? record.Pending.length : 0;
        });
        return {
    ...
    captureNcofiBaseline = async (auditIdOverride = null) => {
        const records = this.cloneNcofiRecords(this.props?.data?.audits?.ncofiRecords);
        this.ncofiBaseline = records;
        const targetAuditId = auditIdOverride ?? this.state.auditId ?? this.props?.navigation?.state?.params?.AuditID;
        if (targetAuditId) {
    ...
    componentDidMount() {
        ...
        this.LongTask();
        this.fetchData();
        this.captureNcofiBaseline(this.props?.navigation?.state?.params?.AuditID).catch(error => console.log('capture baseline error', error));
        //console.log(this.state.checkPointsDetails, 'lostArr');
    }

    // Handles prop/state changes that need follow-up work
    componentDidUpdate(prevProps, prevState) {
        const prevRecords = prevProps && prevProps.data && prevProps.data.audits && prevProps.data.audits.ncofiRecords;
        const currentRecords = this.props && this.props.data && this.props.data.audits && this.props.data.audits.ncofiRecords;
    ...
    fetchData = async () => {
        try {
            ...
            console.log('NCSettingValue in CheckPointDemo:', ncofisettingvalue, 'enabled:', this.isNcofiFeatureEnabled(ncofisettingvalue));
    ...
    LongTask() {
        ...
        console.log(this.props.data.audits.auditRecords[0].CheckpointLogic, 'auditdata');
        ...
        const auditIdParam =
            this.props?.route?.params?.AuditID || this.props?.route?.params?.Check?.AuditID || this.props?.data?.audits?.auditRecords?.[0]?.AuditId;
        ...
        const formIdNavigate =
            this.props?.route?.params?.FormIdNavigate ?? this.props?.route?.params?.FormId ?? this.props?.route?.params?.ChecklistHeading?.FormId;
        ...
        this.setState(
            ...
            () => {
                ...
                for (var i = 0; i < auditRecords.length; i++) {
                    if (auditRecords?.[i]?.AuditId == this.state.auditId) {
                        for (var j = 0; j < auditRecords?.[i]?.Listdata.length; j++) {
                            console.log('+++', auditRecords?.[i]?.Listdata?.[j]?.ParentId, auditRecords?.[i]?.Listdata?.[j]?.FormId);
                            const parentId = auditRecords?.[i]?.Listdata?.[j]?.ParentId;
                            const formId = auditRecords?.[i]?.Listdata?.[j]?.FormId;
                            const matchesTemplate = String(this.state.ChecklistTemplateId) === String(parentId);
                            const matchesForm = !this.state.FormIdNavigate || String(this.state.FormIdNavigate) === String(formId);
                ...
                if (!checkPoints || checkPoints.length === 0) {
                    ...
                    const fallbackCheckpoints =
                        auditRecords
                            ...
                            ?.Listdata?.filter(item => String(item?.ParentId) === String(this.state.ChecklistTemplateId)) || [];
                ...
                //   CheckpointAttachment,
                // );
                let temp = this.props.data.audits.auditRecords.findIndex(obj => obj.AuditId === this.state.auditId);
                ...
                if (data) {
                    for (var i = 0; i < data.length; i++) {
                        if (this.state.ChecklistTemplateId == data[i].ParentId && data[i].FormId == this.props?.route?.params?.FormIdNavigate) {
                            // console.log(
                            //   'index==>',
                            //   i,
                            //   temp,
                ...
                if (Array.isArray(checkPoints) && checkPoints.length > 0 && Array.isArray(checkPointList) && checkPointList.length > 0) {
                    const immediateActionByKey = {};

                    checkPoints.forEach(cp => {
                        const key = [cp.FormId, cp.ChecklistTemplateId, cp.ParentId, cp.SerialNo].map(v => String(v ?? '')).join('_');

                        if (cp.immediateAction !== undefined && cp.immediateAction !== null && cp.immediateAction !== '') {
                            immediateActionByKey[key] = cp.immediateAction;
                        }
                    });

                    checkPointList = checkPointList.map(item => {
                        const key = [item.FormID, item.ChecklistTemplateId, item.ParentId, item.SerialNo].map(v => String(v ?? '')).join('_');
                    ...
                    if (checkPoints.length > 0) {
                        for (var i = 0; i < checkPoints.length; i++) {
                            for (var j = 0; j < checkPointList.length; j++) {
                                if (checkPointList?.[j]?.ChecklistTemplateId == checkPoints?.[i]?.ChecklistTemplateId) {
                                    ...
                                    for (var k = 0; k < checkPointList?.[j]?.scoreTypesData.length; k++) {
                                        if (checkPointList?.[j]?.scoreTypesData[k].value == checkPoints?.[i]?.Score) {
                                            showncofistatus = checkPointList?.[j]?.scoreTypesData[k].status;
                                        }
                                    ...
                                    if (this.props.data.audits.ncofiRecords) {
                                        for (var n = 0; n < this.props.data.audits.ncofiRecords.length; n++) {
                                            if (this.props?.route?.params?.AuditID == this.props.data.audits.ncofiRecords[n].AuditID) {
                                                // console.log(
                                                //   'this audit template have nc ofi records..',
                                                // );

                                                var pending_list = this.props.data.audits.ncofiRecords[n].Pending;
                                                if (pending_list) {
                                                    for (var p = 0; p < pending_list.length; p++) {
                                                        if (pending_list[p].AuditID == this.props.data.audits.ncofiRecords[n].AuditID) {
                                                            if (pending_list[p].ChecklistTemplateId == checkPoints?.[i]?.ChecklistTemplateId) {
                                                ...
                                                var uploaded_list = this.props.data.audits.ncofiRecords[n].Uploaded;
                                                if (uploaded_list) {
                                                    for (var p = 0; p < uploaded_list.length; p++) {
                                                        if (this.props.data.audits.ncofiRecords[n].AuditID == this.props?.route?.params?.AuditID) {
                                                            if (uploaded_list[p].ChecklistTemplateId == checkPoints?.[i]?.ChecklistTemplateId) {
                                                                if (
                                    ...
                                    const resolvedFailureCategoryId = this.normalizeId(
                                        checkPoints?.[i]?.FailureCategoryId ?? checkPointList?.[j]?.FailureCategoryId ?? 0,
                                    );

                                    const resolvedFailureReasonId = this.normalizeId(
                                        checkPoints?.[i]?.FailureReasonId ?? checkPointList?.[j]?.FailureReasonId ?? 0,
                                    );
                                    var temppp = '';
                                        ...
                                        if (checkPoints[i].RadioValue == 0) {
                                            if (checkPointList?.[j]?.Status == 0) {
                                                ...
                                                checkPointsDetails.push({
                                                    ...
                                                    Score: checkPoints?.[i]?.Score == 'N/A' ? -1 : checkPoints?.[i]?.Score,
                                                    ...
                                                    FileSize: checkPoints?.[i].FileSize ? checkPoints?.[i]?.FileSize : 0,
                                                    isScoreValid: true,
                                                    scoreInvalidMsg: '',
                                                    ...
                                                    FailureCategoryId: checkPointList?.[j]?.FailureCategoryId,
                                                ...
                                                console.log('**checkpoints details**03', checkPoints[i]);
                        ...
                        for (var i = 0; i < checkPointList.length; i++) {
                            ...
                            if (checkPointsDetails[i].Score === '-2') {
                                console.log(checkPointsDetails?.[i]?.Score, 'enteringcheckconsole');
                                this.setState({
                ...
                this.setState(
                    {
                        ...
                        ischeckLPA: this.props?.route?.params?.AuditProgramId == -1 ? true : false,
                    ...
                    () => {
                        ...
                        console.log('Load: checkPointsDetails', temppp, this.state.checkpointList);
                        this.captureNcofiBaseline().catch(error => console.log('capture baseline error', error));
    ...
    componentWillUnmount() {
        ...
        if (this.state.isUnsavedData) {
            this.restoreBaselineNCOFI().catch(error => console.log('CheckPointDemo unmount restore error', error));
        }
    }
    ...
    componentWillReceiveProps(props) {
        ...
        const CurrentPage = navRoutes.length > 0 ? navRoutes[navRoutes.length - 1]?.routeName : this.props?.route?.name;
            ...
            if (this.state.attachSelectedItem) {
                ...
                if (cameraCapture.length > 0 && checkPointsDetails.length > 0) {
                    ...
                    this.setState(
                        ...
                        async () => {
                            console.log('Checkpoint page - checkPointsDetails', this.state.checkPointsDetails, cameraCapture);
        ...
        if (this.props.data.audits.ncofiRecords) {
            ...
            if (ncofi_details.length > 0 && originalCheckPoints.length > 0) {
                    ...
                    for (let j = 0; j < ncofi_details.length; j++) {
                        if (ncofi_details[j].AuditID == checkpointDetail.AuditId) {
                            ...
                            for (let k = 0; k < pending_ncofi.length; k++) {
                                if (pending_ncofi[k].AuditID == checkpointDetail.AuditId && pending_ncofi[k].ChecklistTemplateId == templateId) {
                ...
                this.setState(
                    ...
                    () => {
                        console.log('Checkpoint page - checkPointsDetails --nc/ofi', this.state.checkPointsDetails);
                        this.countStatistics(this.state.checkPointsDetails);
                    },
                );
            }
    ...
    updatecheckpointvalues_new = () => {
        ...
        const checkpointsToValidate = checkPointsDetailsSnapshot
            ...
            .filter(({ checkpoint, index }) => this.shouldValidateCheckpointOnSave(checkpoint, index));
                    ...
                    this.setState(
                        ...
                        () => {
                            ...
                            const routeAuditId =
                                ...
                                this.props?.route?.params?.Check?.AuditId;
                            const activeAuditId = this.state.auditId || routeAuditId || (auditRecordsOrg?.[0]?.AuditId ?? '');
                            console.log('[CPD] activeAuditId used for save', activeAuditId, 'state auditId:', this.state.auditId);
                            var listData = [];
                            ...
                            for (var p = 0; p < auditRecordsOrg.length; p++) {
                                ...
                                if (auditRecordsOrg[p].AuditId == activeAuditId) {
                                    if (auditRecordsOrg[p].Listdata) {
                                        if (auditRecordsOrg[p].Listdata.length > 0) {
                                            for (var q = 0; q < auditRecordsOrg[p].Listdata.length; q++) {
                                                console.log('org data====>', auditRecordsOrg[p].Listdata[q]);
                                                console.log('org data====>', auditRecordsOrg[p].Listdata[q]);
                                                ...
                                                listDataArr.push({
                                                    ...
                                                    immediateAction: auditRecordsOrg[p].Listdata[q].immediateAction,
                                                    deleteallattachment:
                                    ...
                                    for (var i = 0; i < listDataArr.length; i++) {
                                        ...
                                        const listTemplateId = normalizeKey(listDataArr[i].ChecklistTemplateId);
                                        const listParentId = normalizeKey(listDataArr[i].ParentId);

                                        for (var j = 0; j < checkPointsDetails.length; j++) {
                                            const checkpointFormId = normalizeKey(checkPointsDetails[j].FormId);
                                            const checkpointTemplateId = normalizeKey(checkPointsDetails[j].ChecklistTemplateId);
                                            const checkpointParentId = normalizeKey(checkPointsDetails[j].ParentId);
                                            ...
                                            if (
                                                ...
                                                listDataArr[i] = {
                                                    ...
                                                    SerialNo: listDataArr[i].SerialNo ?? checkPointsDetails[j].SerialNo,
                                                    ...
                                                    ChecklistTemplateId: listDataArr[i].ChecklistTemplateId,
                                                    ...
                                                    immediateAction: checkPointsDetails[j].immediateAction,
                                                    FailureCategoryId: checkPointsDetails[j].FailureCategoryId,
                                                    // FailureCategoryName:
                                                    //   checkPointsDetails[j].FailureCategoryName ||
                                                    //   this.failureCategoryText(
                                                    //     checkPointsDetails[j].FailureCategoryId,
                                                    //   ),
                                                    FailureReasonId: checkPointsDetails[j].FailureReasonId,
                                                    // FailureReasonName:
                                    ...
                                    let FailureCategoryList = auditRecordsOrg[p].CheckpointLogic.FailureCategory;
                                    let FailureReasonList = auditRecordsOrg[p].CheckpointLogic.FailureReason;
                                    let ImmediateActionList = auditRecordsOrg[p].CheckpointLogic.ImmediateAction;
                                    let LPAApproachList = auditRecordsOrg[p].CheckpointLogic.LPAApproach;
                                    let ScoreTypeList = auditRecordsOrg[p].CheckpointLogic.ScoreType;
                                    let newArray = {
                                    ...
                                    const filteredArr = listDataArr.reduce((acc, current) => {
                                        const x = acc.find(item => item.ChecklistTemplateId === current.ChecklistTemplateId);
                                        if (!x) {
                                    ...
                                    console.log('FormID===>', this.props?.route?.params?.FormIdNavigate);
                                    //Assign Empty checkpoints to Store
                                    let checklistpropdata = [];

                                    for (let m = 0; m < auditRecordsOrg[p].CheckListPropData.length; m++) {
                                        ...
                                        if (chckpropdata.CompLevelId === 3) {
                                            let emptycount = listDataArr.filter(
                                                item =>
                                                    item.FormId === FormId && item.ParentId.toString() === ChecklistTemplateId && item.Score === '-2',
                                            );
                            ...
                            setTimeout(() => {
                                ...
                                var checkPoints = null;
                                for (var i = 0; i < auditRecords.length; i++) {
                                    if (auditRecords?.[i]?.AuditId == this.props?.route?.params?.AuditID) {
                                        if (auditRecords?.[i]?.Listdata) checkPoints = auditRecords?.[i]?.Listdata;
                                    }
                                }
    ...
    popupModal(checkPointDetail) {
        ...
        console.log('checkPointDetail.show_nc_ofi_status' + checkPointDetail.show_nc_ofi_status);
        ...
        this.setState(
            ...
            () => {
                ...
                console.log('checking Details::::----Status_nc_ofi', this.state.Status_nc_ofi);
            },
        );
    }
    ...
    removeAttachment = (index, attach, cpIndex) => {
        ...
        let FilesPath = '/' + RNFetchBlob.fs.dirs.DocumentDir + '/' + (Platform.OS == 'ios' ? 'IosFiles' : 'AuditFiles');
        ...
        for (var j = 0; j < checkPointsDetails.length; j++) {
            ...
            //   continue;
            // }
            let attachment = checkPointsDetails[j].AttachmentList.filter(item => item.id === attach.id);
                ...
                if (RNFetchBlob.fs.isDir(FilesPath)) {
                    RNFetchBlob.fs
                        ...
                        .then(() => {
                            ...
                            var newAttachmentList = checkPointsDetails[j].AttachmentList.filter(item => item.id !== attachment[0].id);
    ...
    navigateTo(id) {
        ...
        var data = null;
        const selectedChecklist = this.state.selectedindex || this.state.checkPointsDetails?.[this.state.ActiveId];
        console.log(
        ...
        if (templateId && (id === 'NC' || id === 'OFI')) {
            const availabilityUpdates = id === 'NC' ? { nc_available_status: true } : { ofi_avialable_status: true };
        ...
        if (pendingList) {
            ...
            for (var i = 0; i < pendingList.length; i++) {
                ...
                if (
                    ...
                    pendingList[i].ChecklistTemplateId == this.state.ncofiPassTemplateId &&
        ...
        if (data == null && uploadedList) {
            ...
            for (var i = 0; i < uploadedList.length; i++) {
                if (uploadedList[i].ChecklistTemplateId == this.state.ncofiPassTemplateId && uploadedList[i].CheckNC == checkNC) {
                    isNCOFIExists = true;
                    isUploaded = true;
            ...
            if (isNCOFIExists) {
                ...
                var selectedItems = uploadedData.ElementId == '0' ? [] : uploadedData.ElementId ? uploadedData.ElementId.split(',') : [];
                var selectedItemsProcess = uploadedData.ProcesssId == '0' ? [] : uploadedData.ProcesssId ? uploadedData.ProcesssId.split(',') : [];
                ...
                data = {
                    ...
                    uniqueNCkey:
                        uploadedData.uniqueNCkey !== undefined && uploadedData.uniqueNCkey !== null ? uploadedData.uniqueNCkey : Moment().unix(),
    ...
    isVdaAudit() {
        try {
            ...
            const activeAuditId =
                this.state?.auditId ?? this.props?.route?.params?.AuditID ?? this.props?.route?.params?.Check?.AuditID ?? auditRecords[0]?.AuditId;

            const targetRecord = auditRecords.find(record => record && String(record.AuditId) === String(activeAuditId)) || auditRecords[0];
            ...
            if (rawVda === true || rawVda === 'true' || rawVda === 'TRUE' || rawVda === 1 || rawVda === '1') {
                return true;
            }

            // Fallback: infer VDA from template/program names when the flag is not set
            const templateName = (targetRecord.AuditTemplateName || targetRecord.Formname || targetRecord.AuditProgramName || '')
                .toString()
                .toUpperCase();
    ...
    requiresNcRemark(checkpoint) {
        ...
        if (selection === 'UNKNOWN') {
            return this.hasRadioSelection(checkpoint) && this.isFlagEnabled(checkpoint?.RemarkforNc);
        }
    ...
    requiresNcAttachment(checkpoint) {
        ...
        if (selection === 'UNKNOWN') {
            return this.hasRadioSelection(checkpoint) && this.isFlagEnabled(checkpoint?.AttachforNc);
        }
    ...
    requiresOfiRemark(checkpoint) {
        ...
        if (selection === 'UNKNOWN') {
            return this.hasRadioSelection(checkpoint) && this.isFlagEnabled(checkpoint?.RemarkforOfi);
        }
    ...
    requiresOfiAttachment(checkpoint) {
        ...
        if (selection === 'UNKNOWN') {
            return this.hasRadioSelection(checkpoint) && this.isFlagEnabled(checkpoint?.AttachforOfi);
        }
    ...
    isRemarkMandatory(checkpoint) {
        ...
        if (['NO', 'NOTOK', 'FALSE'].includes(selection)) {
            return this.requiresNcRemark(checkpoint) || this.requiresOfiRemark(checkpoint);
        }
    ...
    isAttachmentMandatory(checkpoint) {
        ...
        if (['YES', 'OK', 'TRUE'].includes(selection)) {
            ...
            requiresAttachment = this.requiresNcAttachment(checkpoint) || this.requiresOfiAttachment(checkpoint);
    ...
    isServerMandatory(checkpoint, index) {
        ...
        const fallbackList = Array.isArray(this.state.checkpointList) ? this.state.checkpointList : [];
        const fallbackItem = typeof index === 'number' && index >= 0 && index < fallbackList.length ? fallbackList[index] : null;
    ...
    hasAttachmentValue(checkpoint) {
        ...
        const list = Array.isArray(checkpoint.AttachmentList) ? checkpoint.AttachmentList : [];
        ...
        const hasListAttachment = list.some(item => {
            ...
            if ((item.Docid && item.Docid !== 0) || (item.AttachmentId && item.AttachmentId !== 0)) {
                return true;
            }
        ...
        const fallbackAttachment = [checkpoint.Attachment, checkpoint.FileName, checkpoint.File]
    ...
    isNcRequirementMandatory(checkpoint) {
        ...
        return this.requiresNcAttachment(checkpoint) || this.requiresNcRemark(checkpoint);
    }
    ...
    isOfiRequirementMandatory(checkpoint) {
        ...
        return this.requiresOfiAttachment(checkpoint) || this.requiresOfiRemark(checkpoint);
    }
    ...
    shouldValidateCheckpointOnSave(checkpoint, index) {
        ...
        const activeIndexNumber = typeof activeIndexRaw === 'number' ? activeIndexRaw : parseInt(activeIndexRaw, 10);
        const normalizedActiveIndex = Number.isNaN(activeIndexNumber) ? 0 : activeIndexNumber;
    ...
    validateCheckpointRequirements(checkpoint) {
        ...
        if (this.isAttachmentMandatory(checkpoint) && !this.hasAttachmentValue(checkpoint)) {
    ...
    blockRequirement(index, missingType, extraState = {}) {
        const message = missingType === 'attachment' ? 'Please add Attachment to proceed further' : 'Please add Remarks to proceed further';
    ...
    clearNcofiRecordsForTemplates = templateIdList => {
        const activeAuditId = this.state.auditId ?? this.props?.navigation?.state?.params?.AuditID ?? null;
        ...
        const currentRecords = this.cloneNcofiRecords(this.props?.data?.audits?.ncofiRecords);
        ...
        const updatedRecords = currentRecords.map(record => {
            const recordAuditId = record?.AuditID ?? record?.AuditId ?? record?.auditId ?? null;
            if (recordAuditId === undefined || recordAuditId === null || String(recordAuditId) !== String(activeAuditId)) {
                return record;
            }

            const pendingEntries = Array.isArray(record?.Pending) ? record.Pending : [];

            const filteredPending = pendingEntries.filter(item => {
                const templateId = item?.ChecklistTemplateId ?? item?.ChecklistTemplateID ?? null;
                if (templateId === undefined || templateId === null) {
                    return true;
                }
        ...
        const auditRecord = updatedRecords.find(record => String(record?.AuditID ?? record?.AuditId ?? record?.auditId) === String(activeAuditId));

        const pendingList = Array.isArray(auditRecord?.Pending) ? auditRecord.Pending : [];

        const hasPendingNc = pendingList.some(item => {
            const categoryCandidate = item?.Category ?? item?.category ?? item?.CheckNC ?? item?.checkNc ?? item?.checkNC;
        ...
        persistAuditSummarySnapshot(updatedRecords, activeAuditId).catch(error => console.log('clearCheckpoints persist error', error));
    ...
    declineNcRemoval = () => {
        ...
        this.setState(prevState => {
            ...
            return {
                ...
                dialogVisibleNCR: false,
                isUnsavedData: true,
                dropdownnotokvalue: previousRecord?.RadioValue ?? prevState.dropdownnotokvalue,
    ...
    isFailedCheckpoint = items => {
        var checkPointsDetails = this.state.checkPointsDetails;
        for (var i = 0; i < checkPointsDetails.length; i++) {
            if (checkPointsDetails[i].ChecklistTemplateId == items.ChecklistTemplateId && checkPointsDetails[i].RadioValue == 10) {
                return true;
            }
        }
        return false;
    ...
    checkFileAlreadyExist = (checkPointsDetails, items, response) => {
        for (var i = 0; i < checkPointsDetails.length; i++) {
            ...
            if (checkPointsDetails[i].ChecklistTemplateId == items.ChecklistTemplateId && checkPointsDetails[i].FormId === items.FormID) {
                var fileExist = checkPointsDetails[i].AttachmentList.filter(item => item.FileName === response.name);
        ...
        for (var i = 0; i < checkPointsDetails.length; i++) {
            ...
            if (checkPointsDetails[i].ChecklistTemplateId == items.ChecklistTemplateId && checkPointsDetails[i].FormId === items.FormID) {
                ...
                AttachmentList.push({
                    ...
                    FormId: items.FormID,
                    FileUri: Platform.OS === 'ios' ? decodeURIComponent(finalpath) : finalpath,
    ...
    doCompress = async response => {
        let FilesPath = RNFetchBlob.fs.dirs.DocumentDir + '/' + (Platform.OS == 'ios' ? 'IosFiles' : 'AuditFiles');
        const fileuri = Platform.OS == 'ios' ? decodeURIComponent(response.uri.slice(6)) : response.uri;
        var fileName = 'file_' + Moment().unix() + '.' + response.name.substring(response.name.lastIndexOf('.') + 1);
    ...
    attachFiles = async () => {
        ...
        try {
            ...
            if (response) {
                ...
                if (this.checkFileAlreadyExist(checkPointsDetails, items, response) === true) {
                    this.setState(
                        {
                            CheckAttach: false,
                            checkPointsDetails: checkPointsDetails,
    ...
    clearCheckpoints = () => {
        this.setState(
            prevState => {
                ...
                const ncStatus = templateIdsForClear.length > 0 ? this.clearNcofiRecordsForTemplates(templateIdsForClear) : null;

                const ncAvailability = ncStatus && typeof ncStatus.hasPendingNc === 'boolean' ? ncStatus.hasPendingNc : previousNcAvailable;
                const ofiAvailability = ncStatus && typeof ncStatus.hasPendingOfi === 'boolean' ? ncStatus.hasPendingOfi : previousOfiAvailable;
    ...
    failurereasonArray(value) {
        ...
        const categoryId = value === null || value === undefined ? null : parseInt(value, 10);
        const results = (this.state.FailureReasonStateList || []).filter(obj => {
    ...
    // }
    failureReasonText(value) {
        const Failres_value = value === null || value === undefined ? null : parseInt(value, 10);
        const LPAdrop_Arr = Array.isArray(this.state.FailureReasonStateList) ? this.state.FailureReasonStateList : [];
        const match = LPAdrop_Arr.find(element => {
    ...
    syncFailureLabels() {
        ...
        const updated = checkPointsDetails.map(entry => {
            const categoryName = entry.FailureCategoryName || this.failureCategoryText(entry.FailureCategoryId);
            const reasonName = entry.FailureReasonName || this.failureReasonText(entry.FailureReasonId);
            if (entry.FailureCategoryName !== categoryName || entry.FailureReasonName !== reasonName) {
                hasChanges = true;
                return {
                    ...entry,
        ...
        NetInfo.fetch().then(isConnected => {
            if (isConnected.isConnected) {
                for (var j = 0; j < checkpointList.length; j++) {
                    ...
                    if (checkpoint.FormId == attach.FormId && checkpoint.ChecklistTemplateId == attach.ChecklistTemplateID) {
                        let AttachmentList = [];
    ...
    render_loader(minHeight = 180) {
        return (
            <View style={[styles.loaderContainer, { minHeight }]}>
    ...
    async ncofisetting(value) {
        const storedValue = this.state.ncofisettingvalue || (await AsyncStorage.getItem('NCSettingValue'));
        ...
        return (
            <View>
                {(attach.Attachment === 'EMPTY' || attach.Attachment === 'FAILED') && (
                    <Icon style={styles.downloadOverlayIcon} name="download" size={35} color="#888584" />
                )}
                <Icon name={icon} size={65} color="black" style={styles.attachmentIconLarge} />
                {attach.Attachment === 'FAILED' && <Text style={styles.attachmentFailedLabel}>Retry</Text>}
            </View>
        );
    }
    ...
    renderStatus = (checkpoint, scoreTypes) => {
        ...
        if (checkpoint.Score_Type === 3) {
            ...
            status = scoreStatus >= 0 && scoreStatus <= 1 && checkpoint.Scoretext !== '' ? '0' : scoreStatus === 4 ? '1' : '';
        } else {
            status =
                checkpoint.IsCorrect == 1 && checkpoint.RadioValue != 11 ? '1' : checkpoint.IsCorrect == 0 && checkpoint.RadioValue != 11 ? '0' : '';
        }
    ...
    handleNotOkClick = () => {
        ...
        const bundleEntry = {
            requiretext: '-',
            NonConfirmity: auditProgramName === 'lpa' ? sanitizedChecklistName : '-',
            ...
            Category: 'NC',
            uniqueNCkey: selectedChecklist.uniqueNCkey === undefined ? Moment().unix() : selectedChecklist.uniqueNCkey,
            selectedItems: [],
        ...
        const updatedRecords = ncRecords.map(record => {
            ...
            const pendingEntries = Array.isArray(record.Pending) ? [...record.Pending, bundleEntry] : [bundleEntry];
            ...
            <View style={styles.mainContainer}>
                ...
                {this.state.isContentLoaded == false ? (
                        ...
                        {this.props.data.audits.smdata !== 2 && this.props.data.audits.smdata !== 3 ? (
                            <View style={styles.statistics}>
                                <View style={styles.statCard1}>
                                    ...
                                    <Text style={styles.statValue}>{this.state.totalCheck}</Text>
                                </View>

                                {!this.isVdaAudit() && (
                                    <View style={styles.statCard3}>
                                        <Text style={styles.statLabel}>{strings.Asterisk_Questions}</Text>
                                                            ...
                                                            <View style={styles.questionBlockRow}>
                                                                <View style={styles.questionBlockTextCol}>
                                                                    <View style={styles.checkpointTitleRow}>
                                                                        ...
                                                                        {this.state.checkPointsDetails[index].ofi_avialable_status ? (
                                                                            <View style={styles.targetIconWrapper}>
                                                                                <Icon name="target" size={12} color="yellow" />
                                                                            </View>
                                                                        ) : null}
                                                                ...
                                                                <View style={styles.requirementIconCol}>
                                                                    {hasOutstandingRequirement ? (
                                                                        <View
                                                                            style={[
                                                                                styles.requirementIconWrapperSmall,
                                                            ...
                                                            <View style={[styles.checkpointContentColumn, { flex: 1 }]}>
                                                                {item.ansType == 'M1' && item.scoreType !== 3 ? ( //Radio button
                                                                    <View style={styles.boxsecRadio}>
                                                                        <RadioForm
                                                                            key={`radio-${item.ChecklistTemplateId || index}-${
                                                                                this.state.radioResetKey
                                                                            }`}
                                                                            ...
                                                                            initial={
                                                                                ...
                                                                                this.state.checkpointList.length > 0
                                                                                    ? parseInt(this.state.checkPointsDetails[index].RadioValue) ==
                                                                                          9 || parseInt(this.state.checkpointList[index].Status) == 1
                                                                                        ? 0
                                                                                        : parseInt(this.state.checkPointsDetails[index].RadioValue) ==
                                                                                              10 ||
                                                                                          parseInt(this.state.checkpointList[index].Status) == 0
                                                                                        ? 1
                                                                                        : -1
                                                                                    : -1
                                                                            }
                                                                                        ...
                                                                                        if (value == item.correctAnswer) {
                                                                                            ...
                                                                                            if (
                                                                                                ...
                                                                                                for (var j = 0; j < item.scoreTypesData.length; j++) {
                                                                                                    if (
                                                                                                        item.ChecklistTemplateId ==
                                                                                                            item.scoreTypesData[j].templateId &&
                                                                                                        checkPointsDetails[i].Score ==
                                                                                                        ...
                                                                                                        if (item.scoreTypesData[j].status == '1') {
                                                                                                            ...
                                                                                                            item.scoreTypesData[j].status == '2' ||
                                                                                                            item.scoreTypesData[j].status == '3'
                                                                                                        ) {
                                                                            ...
                                                                            initial={
                                                                                ...
                                                                                this.state.checkpointList.length > 0
                                                                                    ? parseInt(this.state.checkpointList[index].Status) == 1 ||
                                                                                      parseInt(this.state.checkPointsDetails[index].RadioValue) == 12
                                                                                        ? 0
                                                                                        : parseInt(this.state.checkpointList[index].Status) == 0 ||
                                                                                          parseInt(this.state.checkPointsDetails[index].RadioValue) ==
                                                                                              13
                                                                    ...
                                                                    <View style={styles.boxsecRadio}>
                                                                        <RadioForm
                                                                            ...
                                                                            initial={
                                                                                ...
                                                                                this.state.checkPointsDetails.length > 0
                                                                                    ? parseInt(this.state.checkpointList[index].Status) == 1 ||
                                                                                      parseInt(this.state.checkPointsDetails[index].RadioValue) == 9
                                                                                        ? 0
                                                                                        : parseInt(this.state.checkpointList[index].Status) == 0 ||
                                                                                          parseInt(this.state.checkPointsDetails[index].RadioValue) ==
                                                                                              10
                                                                                        ? 1
                                                                                        : parseInt(this.state.checkpointList[index].Status) == 2 ||
                                                                                          parseInt(this.state.checkPointsDetails[index].RadioValue) ==
                                                                                              11
                                                                                        ? 2
                                                                                        : -1
                                                                            ...
                                                                            onPress={value => {
                                                                                ...
                                                                                {
                                                                                    value === 10 ? this.handleNotOkClick() : this.ncofisetting(value);
                                                                                    //console.log('==--->', value);
                                                                                    //console.log('v+', value);
                                                                                        ...
                                                                                        if (
                                                                                            ...
                                                                                            if (value == item.correctAnswer) {
                                                                                                ...
                                                                                                if (checkPointsDetails[i].IsNCAllowed == 1) {
                                                                                                    ...
                                                                                                    for (var k = 0; k < dataArr.length; k++) {
                                                                                                        if (
                                                                                                            dataArr[k].AuditID == this.state.auditId
                                                                                                        ) {
                                                                                                            for (
                                                                                                                var j = 0;
                                                                                                                j < dataArr[k].Pending.length;
                                                                                                ...
                                                                                                if (
                                                                                                    ...
                                                                                                    for (
                                                                                                        ...
                                                                                                        if (
                                                                                                            ...
                                                                                                            if (
                                                                                                                item.scoreTypesData[j].status == '1'
                                                                                                            ) {
                                                                                                                checkPointsDetails[i].IsNCAllowed = 1;
                                                                                                                break;
                                                                                                            } else if (
                                                                    ...
                                                                    <View style={styles.boxsecRadio}>
                                                                        <RadioForm
                                                                            ...
                                                                            initial={
                                                                                ...
                                                                                this.state.checkPointsDetails.length > 0
                                                                                    ? parseInt(this.state.checkpointList[index].Status) == 1 ||
                                                                                      parseInt(this.state.checkPointsDetails[index].RadioValue) == 14
                                                                                        ? 0
                                                                                        : parseInt(this.state.checkpointList[index].Status) == 0 ||
                                                                                          parseInt(this.state.checkPointsDetails[index].RadioValue) ==
                                                                                              15
                                                                ...
                                                                <View
                                                                    style={
                                                                        ...
                                                                        (item.scoreType == 1 && parseInt(item.maxScore) == 0) ||
                                                                        (item.scoreType == 2 && this.state.checkPointsDetails[index].Score == '') ||
                                                                        ...
                                                                        {this.state.checkPointsDetails[index] ? (
                                                                            this.state.checkPointsDetails[index].Score != '' ? (
                                                                                <View style={styles.scoreRow}>
                                                                                    <Text style={styles.scoreLabel}>{/* {strings.Score} */}</Text>
                                                                                    {item.scoreType == 1 ? (
                                                                                        <Text style={styles.scoreValue}>
                                                                                            {Math.round(this.state.checkPointsDetails[index].Score)}
                                                                                        </Text>
                                                                                    ) : null}
                                                                                </View>
                                                                            ) : (
                                                                                ...
                                                                                <Slider
                                                                                    value={
                                                                                        this.state.checkPointsDetails[index].Score == ''
                                                                                            ...
                                                                                            : Number(this.state.checkPointsDetails[index].Score)
                                                                                    }
                                                                                    ...
                                                                                    onSlidingComplete={value => {
                                                                                        ...
                                                                                        if (
                                                                                            ...
                                                                                            Math.round(value) >= parseInt(item.minScore)
                                                                                        ) {
                                                                                            isValid = true;
                                                                                        }
                                                                                        if (!isValid) {
                                                                                ...
                                                                                this.props.data.audits.smdata !== 3 ? (
                                                                                    <Dropdown
                                                                                        ...
                                                                                        onChangeText={(value, index, data) => {
                                                                                            ...
                                                                                            const text =
                                                                                                value == strings.Please_select ? '-3' : value;
                                                                                            //console.log('Text', text);

                                                                                            if (text == strings.Please_select) {
                                                                                                this.setState({
                                                                                            ...
                                                                                            if (text !== strings.Please_select) {
                                                                                                ...
                                                                                                for (var i = 0; i < checkPointsDetails.length; i++) {
                                                                                                    if (
                                                                                                            ...
                                                                                                            if (
                                                                                                                item.ChecklistTemplateId ==
                                                                                                                    item.scoreTypesData[j]
                                                                                                                        .templateId &&
                                                                                                                text == item.scoreTypesData[j].value
                                                                                                                //item.scoreTypesData[j].id
                                                                                                            ) {
                                                                                                                console.log(
                                                                                                                    'textvalue' +
                                                                                                                        item.scoreTypesData[j].value,
                                                                                                                );
                                                                                                                console.log(
                                                                                                        ...
                                                                                                        checkPointsDetails[i].Scoretext =
                                                                                                            text == 'N/A' ? text : text;
                                                                                                        // : parseInt(text);
                                                                                                        checkPointsDetails[i].Score =
                                                                                                            scoreindex == 'N/A'
                                                                                    ...
                                                                                    <Dropdown
                                                                                        ...
                                                                                        itemTextStyle={{
                                                                                            fontFamily: 'OpenSans-Regular',
                                                                                            backgroundColor:
                                                                                                this.state.checkPointsDetails[index.Score] == 0
                                                                                                    ? 'red'
                                                                                                    : dropdata.color,
                                                                                        }}
                                                                                        selectedItemColor={'black'}
                                                                                        ...
                                                                                        onChangeText={(value, indexvalue, data) => {
                                                                                            ...
                                                                                            console.log(value, indexvalue, 'helloconsolecheck');
                                                                                            const text =
                                                                            ...
                                                                            <View
                                                                                style={
                                                                                    item.scoreTypesData.length == 0
                                                                                        ? styles.scoreTypesHidden
                                                                                        : styles.scoreTypesContainer
                                                                                }>
                                                                                {this.state.checkPointsDetails[index].Score != '' ? (
                                                                                    <View style={styles.scoreRow}>
                                                                                        <Text
                                                                                            style={
                                                                                                this.state.checkPointsDetails[index].isScoreValid ==
                                                                                                true
                                                                                                    ? styles.scoreLabel
                                                                                                    : styles.scoreLabelInvalid
                                                                                            }>
                                                                                            {strings.Score}
                                                                                        </Text>
                                                                                    </View>
                                                                                ) : null}
                                                                                {parseInt(item.maxScore) == 0 && parseInt(item.minScore) == 0 ? (
                                                                                    <Text style={styles.scoreMaxText}>{item.maxScore}</Text>
                                                                                ) : (
                                                                                    <Slider
                                                                                        value={
                                                                                            this.state.checkPointsDetails[index].Score == ''
                                                                                ...
                                                                                parseInt(item.minScore) == 0 ? null : (
                                                                                    <View style={styles.sliderValueRowAlt}>
                                                                                        <Text style={styles.sliderValueText}>
                                                                                            {parseInt(item.minScore)}
                                                                                        </Text>
                                                                                        <Text style={styles.sliderValueText}>
                                                                ...
                                                                {this.state.ischeckLPA ? (
                                                                    <View
                                                                        style={
                                                                            this.state.checkPointsDetails[index].RadioValue == 11 ||
                                                                            this.state.checkPointsDetails[index].RadioValue == 9 ||
                                                                            this.state.checkPointsDetails[index].RadioValue == 14
                                                                                ? styles.boxsecNone
                                                                                : styles.boxsec1
                                                                        }>
                                                                        {this.state.checkPointsDetails[index] ? (
                                                                            this.state.checkPointsDetails[index].Correction != '' ? (
                                                                                <Text style={styles.scoreSmallLabel}>{strings.Correction}</Text>
                                                                            ) : null
                                                                        ) : null}
                                                                        <View
                                                                            style={
                                                                                ...
                                                                                onChangeText={text => {
                                                                                    ...
                                                                                    for (var i = 0; i < checkPointsDetails.length; i++) {
                                                                                        if (
                                                                                            ...
                                                                                            checkPointsDetails[i].Modified = true;
                                                                                            checkPointsDetails[i].isCorrectionEntered =
                                                                                                text.trim() !== '';
                                                                                        }
                                                                                    }
                                                                                    this.setState({
                                                                ...
                                                                {this.state.ischeckLPA ? (
                                                                    <View
                                                                        ...
                                                                        {dropdata.length > 0 &&
                                                                        (this.state.checkPointsDetails[index]?.Correction || '').trim() === '' ? (
                                                                            <Dropdown
                                                                                label={
                                                                                    this.state.checkPointsDetails[index].Approach.toString() == ''
                                                                                        ? ''
                                                                                ...
                                                                                value={
                                                                                    this.state.checkPointsDetails[index].ApproachId !== 0
                                                                                        ? this.approachText(
                                                                                              this.state.checkPointsDetails[index].ApproachId,
                                                                                          )
                                                                                        : strings.Choose_Approach
                                                                                }
                                                                                onChangeText={(value, index, data) => {
                                                                                        ...
                                                                                        if (
                                                                                            ...
                                                                                            for (var j = 0; j < data.length; j++) {
                                                                                                if (value == data[j].value) {
                                                                                                    ...
                                                                                                    checkPointsDetails[i].Modified = true;
                                                                                                    checkPointsDetails[i].Approach = value;
                                                                                                }
                                                                                            }
                                                                                        }
                                                                                    }
                                                                        ...
                                                                        <View style={styles.sectionSpacer}>
                                                                            {console.log(this.state.checkPointsDetails[index], 'helloid6')}
                                                                            <Dropdown
                                                                                label={strings.FailureCategory}
                                                                                value={
                                                                                    this.state.checkPointsDetails[index].FailureCategoryId !== 0
                                                                                        ? this.state.checkPointsDetails[index].FailureCategoryId
                                                                                        : ''
                                                                                }
                                                                        ...
                                                                        <View style={styles.sectionSpacer}>
                                                                            <Dropdown
                                                                                ...
                                                                                value={
                                                                                    this.state.checkPointsDetails[index].FailureReasonId !== 0
                                                                                        ? this.failureReasonText(
                                                                                              this.state.checkPointsDetails[index].FailureReasonId,
                                                                                          )
                                                                                ...
                                                                                onChangeText={(value, index, data) => {
                                                                                    ...
                                                                                    for (var i = 0; i < checkPointsDetails.length; i++) {
                                                                                        if (
                                                                                            ...
                                                                                            if (selected) {
                                                                                                checkPointsDetails[i].FailureReasonId = selected.id;
                                                                                                checkPointsDetails[i].Modified = true;
                                                                                            }
                                                                                        }
                                                                                    }
                                                                ...
                                                                <View style={[styles.boxsecRemark, styles.remarkSectionCard]}>
                                                                    <View style={styles.fullWidth}>
                                                                        ...
                                                                        this.state.checkPointsDetails[index].Score !== 'N/A' ? (
                                                                            <View>
                                                                                <Text style={styles.scoreSmallLabel}>{strings.Immediate_Action}</Text>
                                                                                ...
                                                                                <View style={styles.rowCenter}>
                                                                                    <View style={styles.flexOne}>
                                                                                        <Dropdown
                                                                                            ...
                                                                                            onChangeText={(value, dropIndex, data) => {
                                                                                                var checkPointsDetails =
                                                                                                    this.state.checkPointsDetails;
                                                                                                for (var i = 0; i < checkPointsDetails.length; i++) {
                                                                                                    if (
                                                                                    ...
                                                                                    />
                                                                                    {(this.state.checkPointsDetails[index].immediateAction === 'X' ||
                                                                                        this.state.checkPointsDetails[index].immediateAction ===
                                                                                            '1') && (
                                                                                        <TouchableOpacity
                                                                                            onPress={() => {
                                                                                                let checkPointsDetails =
                                                                                                    this.state.checkPointsDetails;
                                                                                                checkPointsDetails[index].immediateAction = '';
                                                                                                checkPointsDetails[index].Modified = true;
                                                                                                this.setState({
                                                                                                    checkPointsDetails: checkPointsDetails,
                                                                                                    isUnsavedData: true,
                                                                                                });
                                                                        ...
                                                                        <TextInput
                                                                            ...
                                                                            onChangeText={text => {
                                                                                ...
                                                                                this.setState(
                                                                                    { checkPointsDetails, isUnsavedData: true },
                                                                                    async () => {
                                                                                        console.log(
                                                                                            'checkPointsDetails',
                                                                                            this.state.checkPointsDetails,
                    ...
                    <View style={styles.contentLoaderContainer}>
                        ...
                        <Text style={styles.contentLoaderTitle}>{strings.cp_01}</Text>
                        ...
                        <View /* style={styles.modalBody} */>
                            <View style={styles.modalheading}>
                                <View style={styles.centerAlignedRow}>
                                    <Text style={styles.modalHeadingText}>{strings.Make_your_selection}</Text>
                                </View>
                            </View>

                            {this.state.radiovalue_ncofi == 9 || this.state.radiovalue_ncofi == 14 ? null : (this.state.Status_nc_ofi == 1 ||
                                  (this.state.Status_nc_ofi == 2 && this.state.radiovalue_ncofi !== 11)) &&
                              this.state.radiovalue_ncofi !== '' ? (
                                <TouchableOpacity onPress={this.navigateTo.bind(this, 'NC', this.state.radiovalue_ncofi)}>
                              ...
                              this.state.radiovalue_ncofi !== '' &&
                              !(this.state.radiovalue_ncofi == 9 || this.state.radiovalue_ncofi == 14) ? (
                                <TouchableOpacity onPress={this.navigateTo.bind(this, 'NC', this.state.radiovalue_ncofi)}>
                                    <View style={styles.sectionTop}>
                                        <View style={styles.sectionContent}>
                ...
                <Modal
                    ...
                    <View style={styles.ncModal}>
                        <View /* style={styles.modalBody} */>
                            <View style={styles.modalheading}>
                                <View style={styles.centerAlignedRow}>
                                    <Text style={styles.modalHeadingText}>{strings.Confirm}</Text>
                                </View>
                            </View>
                            ...
                            <TouchableOpacity onPress={() => this.handleDiscardUnsavedChanges(this.state.go_home)}>
                                <View style={styles.sectionTopCancel}>
                                    <View style={styles.sectionContent}>
                ...
                <Modal
                    ...
                    <View style={styles.ncModal}>
                        <View /* style={styles.modalBody} */>
                            ...
                            <TouchableOpacity onPress={this.declineNcRemoval}>
                                <View style={styles.sectionTopCancel}>
                                    <View style={styles.sectionContent}>
                                        <Text style={styles.boxContentClose}>{strings.no}</Text>
                                    </View>
                ...
                <Modal
                    ...
                    <View style={styles.modalavatar}>
                        <TouchableOpacity onPress={() => this.setState({ dialogVisibleAttach: false })} style={styles.transparentCloseButton}>
                            <View style={styles.transparentCloseInner}>
                                <Icon style={styles.closeIconOffset} name="x-circle" size={40} color="#fff" />
                            </View>
                        </TouchableOpacity>
