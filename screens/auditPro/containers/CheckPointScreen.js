...
class CheckPointScreen extends Component {
  ...
  logTotalCount = (stage, details) => {
    try {
      if (details === undefined) {
        console.log(`[CheckPointScreen] ${stage}`);
      } else {
        console.log(`[CheckPointScreen] ${stage}`, details);
      }
    } catch (error) {
      console.log('[CheckPointScreen] log error', error);
    }
  };
  ...
  cloneNcofiRecords = records => {
    ...
    try {
      ...
      console.log('CheckPointScreen clone error', error);
      return [];
    }
  };
  ...
  handleDiscardUnsavedChanges = async navigateHome => {
    if (this.state.isUnsavedData) {
      try {
        ...
        console.log('CheckPointScreen discard error', error);
      }
    }
    this.setState(
      {
        ...
        if (this.state.checkMandate === false) {
          ...
          this.setState({ isSaving: true }, () => {
            ...
            setTimeout(() => {
              ...
              const shouldNavigateHome = this.state.go_home;
              this.setState({
                checkPointsValues: checkPoints,
                isSaving: false,
                isUnsavedData: false,
  ...
  componentWillUnmount() {
    if (this.state.isUnsavedData) {
      this.restoreBaselineNCOFI().catch(error =>
        console.log('CheckPointScreen unmount restore error', error)
      )
    }
  }
