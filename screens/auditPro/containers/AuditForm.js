...
class AuditForm extends Component {
    ...
    componentDidMount() {
        ...
        this._navFocusListener = this.props.navigation.addListener('focus', this.loadAuditEditedFlag);
        ...
        try {
            ...
            database.transaction(tx => {
                tx.executeSql(
                    ...
                    [attachment.filename, attachment.obj, attachment.path, 'failed', JSON.stringify(attachment)],
    ...
    async loadAuditEditedFlag() {
        try {
            const AuditID = this.props?.route?.params?.AuditID || this.state.AuditID;
    ...
    componentDidUpdate(prevProps) {
        if (prevProps?.route?.params?.AuditID !== this.props?.route?.params?.AuditID) {
    ...
    displayNCSync(ncofiRecords) {
        ...
        const routeAuditId = this.props?.route?.params?.AuditID ?? this.props?.route?.params?.CreateNCdataBundle?.AuditID;

        if (this.props.data.audits.isAuditing === true && routeAuditId && String(this.state.AuditID) === String(routeAuditId)) {
            this.setState({ notifyRed: true });
        }
    }
    // displayNCSync(ncofiRecords) {
    ...
    getForname() {
        ...
        this.setState({ RefList: ReferenceArr, TempList: TemplateArr, OnlineList: OnlineArr }, () => {
                ...
                if (audits[i].AuditId == this.state.Checkpointpass.AuditID) {
                    if (audits[i].Listdata) {
                        ...
                        for (var kk = 0; kk < FormIds.length; kk++) {
                            ...
                            for (var j = 0; j < audits[i].Listdata.length; j++) {
                                ...
                                NewAttachments =
                                    AttachmentList.length > 0
                                        ? AttachmentList.filter(item => item.Attachment !== 'Downloaded' && item.Attachment !== 'Synced')
                                        : [];

                                if (audits[i].Listdata[j].Modified == true && audits[i].Listdata[j].FormId == FormIds[kk]) {
                                    console.log('FormIds1audits[i]1.Listdata[j]1', audits[i].Listdata[j], NewAttachments);
                                    hasListData = true;
                                    ...
                                    if (
                                        ...
                                        if (NewAttachments.length > 0) {
                                            ...
                                            for (let k = 0; k < NewAttachments.length; k++) {
                                                ...
                                                this.auditAttachments.push({
                                                    ...
                                                    FormId: audits[i].Listdata[j].FormId === -1 ? 0 : audits[i].Listdata[j].FormId,
                                                    // FileContent: audits[i].Listdata[j].FileContent,
                                                    SiteLevelId: 0,
                                                });
                                                console.log('auditAttachmentsvvvvvvvvvvvvv', this.auditAttachments);
                                                console.log('auditAttachments', this.auditAttachments, NewAttachments, '<-->', attachment);
                                            }
                                        }
                                        ...
                                        var score_value = 0;
                                        console.log('consolelogs', audits[i].CheckpointLogic.ScoreType);
                                        if (audits[i].Listdata[j].ScoreType == 3) {
                                            console.log('if Score type ==3 then finding score value.. ');
                                            for (var k = 0; k < audits[i].CheckpointLogic.ScoreType.length; k++) {
                                                if (
                                                    audits[i].CheckpointLogic.ScoreType[k].ChecklistTemplateId ==
                                                        audits[i].Listdata[j].ChecklistTemplateId &&
                                                    audits[i].Listdata[j].Score == audits[i].CheckpointLogic.ScoreType[k].ScoreText
                                                ) {
                                                    score_value = audits[i].CheckpointLogic.ScoreType[k].ScoreValue;
                                                    console.log('score value:' + score_value);
                                                }
                                            }
                                        } else {
                                            console.log('if Score type !=3 then set default score value.. ');
                                            score_value = audits[i].Listdata[j].Score !== 0 ? parseInt(audits[i].Listdata[j].Score) : 0;
                                        }
                                        ...
                                        auditCheckPoints.push({
                                            ChecklistTemplateId: parseInt(audits[i].Listdata[j].ChecklistTemplateId),
                                            ParentId: audits[i].Listdata[j].ParentId ? parseInt(audits[i].Listdata[j].ParentId) : 0,
                                            //AttachmentList: [],
                                            ...
                                            Correction: audits[i].Listdata[j].Correction == '' ? 0 : audits[i].Listdata[j].Correction,
                                            //Approach: audits[i].Listdata[j].Approach === '' ? undefined:audits[i].Listdata[j].Approach,
                                            Approach: audits[i].Listdata[j].ApproachId == '' ? 0 : audits[i].Listdata[j].ApproachId,
                                            //Score: (audits[i].Listdata[j].Score) ? parseInt(audits[i].Listdata[j].Score) : 0,
                                            Score: audits[i].Listdata[j].Score ? audits[i].Listdata[j].Score : 0,
                                            // Score: score_value,
                                            ImmediateAction: audits[i].Listdata[j].immediateAction,
                                            Status: parseInt(audits[i].Status),
                                            FailureCategoryId: audits[i].Listdata[j].FailureCategoryId,
                            ...
                            console.log('FormIds1Checking attachements--->', this.auditAttachments);
                        }
                ...
                auth.syncAuditsToServer(auditRecords, TOKEN, (res, data) => {
                    ...
                    if (data.data) {
                        ...
                        if (data.data.Message === 'Success') {
                            ...
                            if (data.data.Data[0]) {
                                console.log('logsuccesscheckauditstatus-----', data.data.Data[0]);

                                var responseArray = data.data.Data[0];

                                var objRespoonseArray = responseArray.Obj;
                                const splitValues = objRespoonseArray.map(item => item.ObjIdValue.split(','));

                                // Flatten the array of arrays into a single array
                                const flattenedArray = [].concat(...splitValues);
                                console.log('LOGFOROBJJJJCHECKKKKK-------------', flattenedArray);
                                var auditObjArr = [];
                                ...
                                for (var i = 0; i < auditObjArr.length; i++) {
                                    ...
                                    formid =
                                        formid == '0' && (smdata == '1' || smdata == '2') ? -2 : formid == 0 && smdata != '1' ? -1 : parseInt(formid);
                                ...
                                console.log('ChecklistObjectArr', this.checkListObjects, this.auditAttachments);
                                ...
                                for (var i = 0; i < this.auditAttachments.length; i++) {
                                    for (var j = 0; j < this.checkListObjects.length; j++) {
                                        console.log('this.auditAttachments[i].FormId', this.auditAttachments[i].FormId);
                                        console.log('this.checkListObjects[j].formId', this.checkListObjects[j].formId);
                                        console.log('this.auditAttachments[i].Type', this.auditAttachments[i].Type);
                                        console.log('this.auditAttachments[i].Id', this.auditAttachments[i].Id);
                                        console.log('parseInt(this.checkListObjects[j].templateId', parseInt(this.checkListObjects[j].templateId));
                                        if (
                                            // this.auditAttachments[i].FormId ==
                                            // this.checkListObjects[j].formId &&
                                            this.auditAttachments[i].Type == 'CL' &&
                                            this.auditAttachments[i].Id == parseInt(this.checkListObjects[j].templateId)
                                        ) {
                                            console.log('CHECK---------------', this.checkListObjects[j]);
                                            this.auditAttachments[i].Obj = this.checkListObjects[j].obj;
                                            this.auditAttachments[i].Id = parseInt(this.checkListObjects[j].templateId);
                                            this.auditAttachments[i].SiteLevelId = this.checkListObjects[j].siteLevelId;

                                            this.auditAttachments[i].FormId = this.checkListObjects[j].formId;
                                        } else {
                                ...
                                console.log('Request formed after audit formed', this.auditAttachments);
    ...
    docProRequest() {
        return new Promise(async (resolve, reject) => {
            ...
            for (var i = 0; i < this.auditAttachments.length; i++) {
                ...
                let getext = this.auditAttachments[i].filename.substr(this.auditAttachments[i].filename.lastIndexOf('.') + 1);
                loopCount++;
                ...
                this.convertFile(this.auditAttachments[i].File, i, 0).then(res => {
                    ...
                    console.log('jjhjhsdjhsjdhjasdjhajsdh@@@@@@@@@@auditAttachments', attachment);
                    ...
                    if (attachment.Type == 'NC') {
                        ...
                        nobj = combinedArray.filter(
                            item => item.filename.replace(/\s+/g, '') === ngetfname && item.templateId == ntemplateid && item.formId === nformid,
                        );
            ...
            this.checkFileExist(formRequestObj.filepath).then(exist => {
                if (exist) {
                    auth.getdocProAttachment(attachmentsArr, token, (res, data) => {
                        ...
                        console.log('120 formRequestArr response formRequestArr-', formRequestArrPush);
    ...
    handleNCOFIConnection() {
        ...
        const routeParams = this.props?.route?.params || this.props?.navigation?.state?.params || {};

        const resolvedAuditIdRaw =
            this.state.AuditID || this.state.Checkpointpass?.AuditID || routeParams.AuditID || routeParams?.CreateNCdataBundle?.AuditID || null;

        const resolvedAuditId = resolvedAuditIdRaw ? String(resolvedAuditIdRaw) : null;
        ...
        const currentAudit = auditRecords.find(ar => String(ar.AuditId) === resolvedAuditId);
        const defaultCategoryId = currentAudit?.DropDownProps?.Category?.[0]?.CategoryId ?? 0;
        ...
        if (!resolvedAuditId) {
            console.log('[AuditForm] handleNCOFIConnection: no resolved AuditID, skipping payload build.');
        }

        for (var i = 0; i < dataArr.length; i++) {
            console.log('[AuditForm] handleNCOFIConnection compare AuditID record=', dataArr[i].AuditID, 'state=', resolvedAuditId);
            if (resolvedAuditId && String(dataArr[i].AuditID) === String(resolvedAuditId)) {
                for (var j = 0; j < dataArr[i].Pending.length; j++) {
                    ...
                    const isNC = categoryUpper === 'NC' || categoryUpper === '0' || categoryUpper.startsWith('NC');
                    const isOFI = categoryUpper === 'OFI' || categoryUpper === '1' || categoryUpper.indexOf('OFI') !== -1;

                    // Log what categoryDrop actually is:
                    console.log('[AuditForm] pendingItem.Category raw=', rawCategory, 'normalized=', categoryUpper);
                    console.log('[AuditForm]   categoryDrop object:', pendingItem.categoryDrop);
                    console.log('[AuditForm]   categoryDrop.id (if exists):', pendingItem.categoryDrop?.id);
                    console.log('[AuditForm]   defaultCategoryId:', defaultCategoryId);

                    const categoryIdToSend = pendingItem.categoryDrop?.id != null ? pendingItem.categoryDrop.id : defaultCategoryId;
                    ...
                    if (isNC) {
                        ...
                        formRequest.push({
                            ...
                            strProcess:
                                dataArr[i].Pending[j].selectedItemsProcess.length > 0 ? dataArr[i].Pending[j].selectedItemsProcess.join(',') : '',
                            ...
                            RequestedBy: dataArr[i].Pending[j].requestDrop ? dataArr[i].Pending[j].requestDrop : 0,
                        ...
                        console.log('[AuditForm] NC payload CategoryIdToSend=', categoryIdToSend);
                        ...
                        formRequest.push({
                            ...
                            ChecklistId: dataArr[i].Pending[j].ChecklistTemplateId === '' ? 0 : parseInt(dataArr[i].Pending[j].ChecklistTemplateId),
                            ElementID: dataArr[i].Pending[j].selectedItems ? dataArr[i].Pending[j].selectedItems.join(',') : 0,
                            ResponsibilityUser: dataArr[i].Pending[j].ResponsibilityUser[0] ? dataArr[i].Pending[j].ResponsibilityUser[0] : 0,
                            NCIdentifier: dataArr[i].Pending[j].ncIdentifier === undefined ? '' : dataArr[i].Pending[j].ncIdentifier,
        ...
        auth.syncNCOFIToServer(datapass, TOKEN, (res, data) => {
            ...
            if (data.data) {
                if (data.data.Message === 'Success') {
                    ...
                    console.log('Request formed after audit formed4545454545', this.auditAttachments);
    ...
    updateFormUploadDetails = () => {
        ...
        for (var i = 0; i < auditListOrg.length; i++) {
            ...
            if (parseInt(auditListOrg[i].ActualAuditId) == parseInt(this.state.Checkpointpass.AuditID)) {
    ...
    updateFormdataRecords() {
        ...
        for (var p = 0; p < auditRecordsOrg.length; p++) {
            if (auditRecordsOrg[p].AuditId == this.state.AuditID) {
                if (auditRecordsOrg[p].Formdata) {
                    if (auditRecordsOrg[p].Formdata.length == this.state.formDetails.length) {
                        auditRecords.push({
                            AuditTypeOrder: auditRecordsOrg[p].AuditTypeOrder,
                            AuditId: auditRecordsOrg[p].AuditId,
                            AuditOrderId: auditRecordsOrg[p].AuditOrderId,
        ...
        if (this.state.formDetails) {
            for (var j = 0; j < this.state.formDetails.length; j++) {
                if (this.state.formDetails[j].FormType != 1) {
                    if (this.state.formDetails[j].Attachmenttype == 0 && this.state.formDetails[j].AttachedDocument != '') {
                        this.isDocsAvail = true;
                        this.auditAttachments.push({
                            Type: 'AR',
                    ...
                    if (data.data) {
                        if (data.data.Message === 'Success') {
                            if (data.data.Data[0].AuditAttachmentStatus) {
                                ...
                                console.log('checking data', data.data.Data[0].AuditAttachmentStatus);

                                var audtFormRespArr = data.data.Data[0].AuditAttachmentStatus.replace('{', '').replace('}', '').split(',');
                                ...
                                console.log('syncAuditFormsToServer auditSiteLevelID', auditSiteLevelID);
                                ...
                                for (var i = 0; i < this.auditAttachments.length; i++) {
                                    for (var j = 0; j < this.formObjects.length; j++) {
                                        if (
                                            ...
                                            this.auditAttachments[i].Id == parseInt(this.formObjects[j].formId)
                                        ) {
                                            this.auditAttachments[i].Obj = this.formObjects[j].obj;
                                            this.auditAttachments[i].Id = parseInt(this.formObjects[j].formId);
                                            this.auditAttachments[i].SiteLevelId = this.formObjects[j].siteLevelId;
                                        }
                                    }
                                }
                ...
                if (parseInt(this.syncStatus) == 4) {
                    ...
                    for (var p = 0; p < auditRecordsOrg.length; p++) {
                        ...
                        auditRecords.push({
                            ...
                            DocRevNo: auditRecordsOrg[p].DocRevNo,
                            AuditRecordStatus:
                                auditRecordsOrg[p].AuditId == this.state.AuditID ? constants.StatusSynced : auditRecordsOrg[p].AuditRecordStatus,
                            AuditResults: auditRecordsOrg[p].AuditResults,
                            AuditProcessList: auditRecordsOrg[p].AuditProcessList,
                            PerformStarted: auditRecordsOrg[p].PerformStarted,
    ...
    viewDocument(fileData, fileName) {
        ...
        if (Platform.OS == 'android') {
            var path = dirs.DownloadDir.replace('/Download', '') + '/Android/data/com.omnex.auditpro/cache/' + fileName.replace(/\s/g, '-');
        } else {
            var path = dirs.DocumentDir + '/' + 'Downloads/' + fileName.replace(/\s/g, '-');
        }
                    ...
                    fs.unlink(path)
                        .then(() => {
                            ...
                            fs.createFile(path, fileData, 'base64')
                                .then(success => {
                                    console.log('File downloaded and stored in internal storage!', success);
    ...
    async checkFilePath() {
        ...
        console.log('this.state.Checkpointpass.AuditID----', this.state.Checkpointpass.AuditID);
        ...
        for (var i = 0; i < audits.length; i++) {
            if (audits[i].AuditId == this.state.Checkpointpass.AuditID) {
                for (var j = 0; j < audits[i].Formdata.length; j++) {
                    ...
                    if (audits[i].Formdata[j].AttachedDocument != '' && audits[i].Formdata[j].Attachmenttype == 0) {
                        console.log('audits[i].Formdata', audits[i].Formdata);

                        if (audits[i].Formdata[j].AttachedDocument !== audits[i].Formdata[j].DocName) {
        ...
        for (var j = 0; j < pushFormpath.length; j++) {
            if (pushFormpath[j].Attachmenttype == 0) {
                let checkPath = await this.isPathExist(pushFormpath[j].AttachedDocument);
                // console.log('checkPath',checkPath)
                    ...
                    <View style={styles.missingModal}>
                        ...
                        <View style={styles.missingBody}>
                            <View style={styles.missingBodyHeader}>
                                <Text style={styles.bodyText1}>{strings.ThfollowingCheckpointFormaredetectedwithmissingattachmentfiles}</Text>
