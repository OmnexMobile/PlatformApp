...
import { ICON_TYPE, OPACITY_ANIMATION, ROUTES } from 'constants/app-constant';
import { AnimatableView, IconComponent, ImageComponent } from 'components';
...
class LoginUIScreen extends Component {
  ...
  constructor(props) {
    ...
    console.log('this.props?.data?.audits?.isDeviceRegistered', this.props?.data?.audits?.isDeviceRegistered)
    this.state = {
      username: '',
      password: '',
      userId: '',
  ...
  multipleAuditUser(ID) {
    ...
    if (Platform.OS == 'android') {
      RNFS.readDir('/data/user/0/com.omnex.auditpro/cache/AuditUser')
        .then(result => {
    ...
    if (Platform.OS == 'android') {
      ...
      RNFS.readFile(path).then(res => {
        ...
        if (LoggedUserDetails[0].audits.userName == null) {
          ...
          console.log('Refillign props1', LoggedUserDetails[0].audits);
...
//     }
//     if (!fcmToken) {
//       fcmToken = await ReactNativeFirebase.messaging().getToken()
//       if (fcmToken) {
...
//   }
  ...
  //   try {
  //     console.log('entersso-.this.state.ssoConfigObj,',this.state.ssoConfigObj);
  //     var additionalParameters = JSON.parse([this.state.ssoConfigObj.discoveryUri.replace(/'/g,"\"")]);
  //     console.log('additionalParameters',additionalParameters);
  //     let myConfig = { 
  //       serviceConfiguration: {
  //         authorizationEndpoint:this.state.ssoConfigObj.issuer,
  //         tokenEndpoint: this.state.ssoConfigObj.issuer,
  //       },
  //       clientId: this.state.ssoConfigObj.clientId,
  //       redirectUrl: this.state.ssoConfigObj.redirectUrl,
  //       scopes: [this.state.ssoConfigObj.scope],
  //       usePKCE: false,
  //       useNonce: false,
  ...
  //     this.loginCall(resultAuth.accessToken, '', {
  //       config: myConfig,
  //       token: {
            ...
            this.setState(
              ...
              () => {
                ...
                if (
                  ...
                  console.log('reach 1 loginCall')
                  // this.loginCall(usrfield, pwdfield);
                  
                } else {
                  console.log('reach 2 loginCall')
    ...
    auth.loginUser(
      ...
      (res, data) => {
        ...
        if (data.data.Success == true) {
          ...
          console.log('default siteid--->', this.props?.data?.audits?.siteId?.length, this.props.data.audits.siteId, data.data.Data[400].Siteid)
          // console.log('default siteid--->', this.props?.data?.audits?.siteId?.length ? this.props.data.audits.siteId : data.data.Data[401].Siteid)
          this.setState(
            {
              userId: data.data.Data[0].UserId.toString(),
              siteId: this.props?.data?.audits?.siteId?.length ? this.props.data.audits.siteId : data.data.Data[400].Siteid,
            ...
            () => {
              ...
              // call below 2 methods later zzz
              // this.getProfileCall(this.state.accessToken)
              // this.getYearAudit()
              this.getProfileCall(this.state.accessToken);
  ...
  onPressSettings = () => {
    ...
    console.log('this.props.data.audits.isDeviceRegistered--->', this.props.data.audits.isDeviceRegistered, this.props)
    ...
    this.props.navigation.navigate(ROUTES.UNREGISTER, {
      openFrom: 'login',
      isDeviceRegistered: this.props.data.audits.isDeviceRegistered,
    });
  };
  ...
  LanguagePress = () => {
    ...
    this.props.navigation.navigate(ROUTES.LANGUAGES);
  };
  _storeToken = () => {
  ...
  render() {
    ...
    console.log(this.state.checkboxSelection, 'this.state.checkboxSelection');
    return (
      <KeyboardAvoidingView>
        <View style={styles.mainContainer}>
          <OfflineNotice />
          {/* <Image source={Images.LoginBack2} style={styles.backgroundImage} /> */}
          <ImageComponent source={IMAGES.loginBack} style={styles.backgroundImage} />
