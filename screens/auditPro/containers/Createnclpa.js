...
class CreateNC extends Component {
    ...
    componentDidMount() {
        ...
        const directParams = this.props?.route?.params || this.props?.navigation?.state?.params || {};
        ...
        if (checklistName === undefined || radioVal === undefined) {
            ...
            const ncRouteFromStore = navRoutes.find(route => route?.routeName === 'CreatencLPA' || route?.routeName === ROUTES.CREATE_NCLPA) || {};
            if (ncRouteFromStore.params) {
                checklistName = checklistName ?? ncRouteFromStore.params.checklistName;
        ...
        if (checklistName === undefined || radioVal === undefined) {
            const navState =
                (this.props.navigation && typeof this.props.navigation.getState === 'function' && this.props.navigation.getState()) ||
                this.props.navigation?.state ||
                {};
            const routes = navState.routes || [];
            const currentRoute = routes.find(route => route?.name === ROUTES.CREATE_NCLPA || route?.routeName === ROUTES.CREATE_NCLPA) || {};

            if (currentRoute.params) {
                checklistName = checklistName ?? currentRoute.params.checklistName;
                radioVal = radioVal ?? currentRoute.params.radiovalue;
            }
        }

        // Normalize radio value so it works when passed as string or number
        const numericRadioVal = typeof radioVal === 'number' ? radioVal : parseInt(radioVal, 10);
        ...
        if (data && data.audits?.auditRecords?.[0]?.CheckListPropData?.[4]?.ChecklistName) {
            this.setState({
                checklistName: data.audits.auditRecords[0].CheckListPropData[4].ChecklistName,
            });
        }
        // this.setState({
    ...
    LongTask() {
        ...
        const params = this.props?.route?.params || this.props?.navigation?.state?.params || {};

        const clauseMandatory = params.NCOFIDetails?.clauseMandatory;
        const shouldInitialize = !this.state.hasInitialized;

        if (!shouldInitialize) {
            if (clauseMandatory !== undefined && clauseMandatory !== this.state.clauseMandatory) {
        ...
        const resolvedType = incomingType ? incomingType : hasPrefillData ? 'EDIT' : 'ADD';
        ...
        for (let i = 0; i < auditRecords.length; i++) {
            const auid = typeof params.NCOFIDetails == 'string' ? params.AuditID : params.NCOFIDetails?.AuditID;
        ...
        if (params.CheckpointRoute) {
            ...
            console.log('CreateNCLPA LongTask params from CheckPointDemo', {
            ...
            this.setState(
                {
                    ...
                    AuditID: typeof params.NCOFIDetails == 'string' ? params.NCOFIDetails : params.NCOFIDetails?.AuditID,
                    ...
                    NCFailure: data ? (data.failureDrop == '0' ? undefined : data.failureDrop) : undefined,
                    nonconfirmityText: data ? data.NonConfirmity : this.state.nonconfirmityText,
                    ...
                    selectedItemsResponse: data && data.ResponsibilityUser ? data.ResponsibilityUser : undefined,
                },
                () => {
                    this.getDropValue();
                },
        ...
        if (processautoid !== '' && processautoid !== null && processautoid !== undefined && resolvedType === 'ADD') {
    ...
    componentWillReceiveProps(props) {
        ...
        if (navigation && typeof navigation.getParam === 'function') {
            ...
            const allParams = props?.route?.params || this.props?.route?.params || navigation?.state?.params || {};
    ...
    getDropDownData(dropdata) {
        ...
        const resolveOption = (current, list) => {
            ...
            const numericId = typeof current === 'number' ? current : parseInt(current, 10);
        ...
        const resolvedCategory = resolveOption(this.state.NCcategoryt, categoryArr);
        const resolvedCategoryWithDefault = resolvedCategory || (categoryArr.length > 0 ? categoryArr[0] : undefined);
        ...
        const requestSource = this.state.NCrequestby || (this.state.ncData ? this.state.ncData.requestDrop : undefined);
    ...
    onSave() {
        ...
        this.setState({ PageLoader: true, isSavebtn: true, isSaved: false }, () => {
            ...
            if (this.state.RouteParam === 'NC') {
                ...
                const needsClauseText = this.state.clauseMandatory === 1 && this.state.RouteParam === 'NC' && !this.state.isLPA;
                ...
                if (hasMandatoryFields) {
                    ...
                    const resolvedCategoryDrop =
                        this.state.NCcategoryt && this.state.NCcategoryt.id
                            ...
                            : this.state.categoryArr && this.state.categoryArr[0] && this.state.categoryArr[0].id
                            ? this.state.categoryArr[0].id
                            : 0;
                    const resolvedUniqueKey =
                        this.state.type === 'EDIT' && this.state.ncData?.uniqueNCkey ? this.state.ncData.uniqueNCkey : Moment().unix();
                            ...
                            if (NCrecords[i].Pending) {
                                for (var j = 0; j < NCrecords[i].Pending.length; j++) {
                                    if (
                                        ...
                                        this.state.ncData.uniqueNCkey == NCrecords?.[i]?.Pending?.[j]?.uniqueNCkey &&
                                        ...
                                        Information.push({
                                            ...
                                            ResponsibilityUser: NCrecords?.[i]?.Pending?.[j]?.ResponsibilityUser,
    ...
    render() {
        ...
        console.log(
            ...
            routes.find(route => route.routeName === ROUTES.CREATE_NCLPA)?.params?.checklistName,
            'tonclpa',
        );
        ...
        const radioValue = routes.find(route => route.routeName === ROUTES.CREATE_NCLPA)?.params?.radiovalue;

        //console.log(radioValue, 'radiofromdemo'); // Output: 15
        // const checklistName = this.props?.route?.name?.find(
        const checklistName = routes.find(route => route.routeName === ROUTES.CREATE_NCLPA)?.params?.checklistName;
        ...
        const requestByOptions = user?.map(item => ({
            label: item?.value,
            value: item?.id,
        }));
        const failureOptions = FailureCategory?.map(obj => ({
            label: obj?.FailureCategoryName || obj?.value,
            value: obj?.FailureCategoryId || obj?.id,
        }));
        ...
        const explicitEdit = this.state.type && typeof this.state.type === 'string' && this.state.type.toUpperCase() === 'EDIT';
        const radioIndicatesEdit = this.state.radioVal === 10 || this.state.radioVal === 15;
        const isEdit = explicitEdit || hasPrefillData || radioIndicatesEdit;
        const pageTitle = isNC ? `${isEdit ? strings.Edit : strings.Upload} NC` : `${isEdit ? strings.Edit : strings.Upload} OFI`;
                    ...
                    <KeyboardAwareScrollView extraHeight={125}>
                            ...
                            <ScrollView showsVerticalScrollIndicator={false}>
                                <View style={{ marginBottom: 50 }}>
                                        ...
                                        {this.state.RouteParam === 'NC' ? (
                                            <View style={styles.input02}>
                                                <InputComponent
                                                    ...
                                                    defaultValue={this.state.nonconfirmityText}
                                                    multiline
                                                    required
                                                    onChangeText={(name, text) => this.setState({ nonconfirmityText: text })}
                                                />
                                            </View>
                                        ) : (
                                            <View style={styles.input02}>
                                                ...
                                                <InputComponent
                                                    ...
                                                    defaultValue={this.state.ofitext}
                                                    multiline
                                        ...
                                        {this.state.RouteParam === 'NC' ? (
                                            <View style={styles.input02}>
                                                <InputComponent
                                                    ...
                                                    defaultValue={radioValue === 15 ? 'N/A' : this.state.objEvidence}
                                                    multiline
                                                    onChangeText={(name, text) => {
                                            ...
                                            <View style={styles.input02}>
                                                ...
                                                <InputComponent
                                                    ...
                                                    label={strings.Objective_Evidence}
                                                    placeholder={strings.Objective_Evidence}
                                                    defaultValue={this.state.objEvidence}
                                                    multiline
                                                    onChangeText={(name, text) => {
                                                        this.setState({ objEvidence: text });
                                                    }}
                                    ...
                                    <View style={styles.div01}>
                                        <View style={[styles.input002]} onPress={() => this.setState({ isVisible: true })}>
                                            ...
                                            <View style={{ flexDirection: 'row' }}>
                                                <TextInput
                                                    ...
                                                    placeholderTextColor={this.state.underline2 == true ? 'red' : '#A6A6A6'}
                                                    textColor="#747474"
                                                    numberOfLines={1}
                                                    value={
                                                        this.state.displayData
                                        ...
                                        <View style={this.state.RouteParam == 'OFI' || this.state.isLPA ? { display: 'none' } : styles.check}></View>
                                    </View>
                                    <View style={styles.div1}>
                                        <View style={{ flex: 1 }}>
                                            <DropdownComponent
                                                ...
                                                value={this.state.NCcategoryt?.id || categoryOptions?.[0]?.value || null}
                                                onChange={value => {
                                                    const selected = this.state.categoryArr?.find(item => item.id === value) || null;
                                                    this.isCheckCategory = !!selected;
                                                    this.setState({ NCcategoryt: selected });
                                                }}
                                            />
                                        </View>
                                    </View>
                                    <View style={styles.div1}>
                                        <View style={{ flex: 1 }}>
                                            <DropdownComponent
                                                ...
                                                value={this.state.NCresponsible?.id || requestByOptions?.[0]?.value || null}
                                                onChange={value => {
                                                    const selected =
                                                        this.state.RequestArr?.find(item => item.id === value) ||
                                                        this.state.UserArr?.find(item => item.id === value) ||
                                                        null;
                                                    this.isCheckUser = !!selected;
                                                    this.setState({ NCresponsible: selected }, () => {
                                    ...
                                    <View style={styles.div1}>
                                        <View style={{ flex: 1, position: 'relative' }}>
                                            {this.state.NCFailure && (
                                                <TouchableOpacity
                                                    onPress={() => this.setState({ NCFailure: undefined })}
                                                    style={{
                                                        position: 'absolute',
                                                        top: -6,
                                                        right: 10,
                                            ...
                                            <DropdownComponent
                                                ...
                                                data={failureOptions}
                                                value={
                                                    this.state.NCFailure?.id ||
                                                    this.state.NCFailure?.FailureCategoryId ||
                                                    failureOptions?.[0]?.value ||
                                                    null
                                                }
                                                onChange={value => {
                                                    const selected =
                                                        this.state.FailureCategory?.find(item => (item.id || item.FailureCategoryId) === value) ||
                                                        null;
                                                    this.isCheckFailure = !!selected;
                                                    this.setState({ NCFailure: selected });
                                                }}
                                            />
                                        </View>
                                    </View>
                                            ...
                                            <View>
                                                ...
                                                <SectionedMultiSelect
                                                    ...
                                                    onSelectedItemsChange={selectedItems => {
                                                        ...
                                                        this.setState({ selectedItemsResponse: selectedItems, UserArr: updatedUserArr }, () => {
                                    ...
                                    <View style={[styles.div1, { display: 'none' }]}>
                                        <View style={styles.input07}>
                                            <DropdownComponent label={strings.Auditee_Approach} data={[]} value={null} editable={false} />
                                        </View>
                                    </View>
                                    <View style={styles.div1}>
                                        {this.state.RouteParam === 'NC' ? (
                        ...
                        this.props.data.audits.smdata != 3 ? ( */}
                                                <InputComponent
                                                    ...
                                                    defaultValue={radioValue === 15 ? 'N/A' : this.state.documentRef}
                                                    multiline
                                                    onChangeText={(name, text) => {
                                            ...
                                            <View style={styles.input02}>
                                                ...
                                                <InputComponent
                                                    ...
                                                    label={strings.Document_reference}
                                                    placeholder={strings.Document_reference}
                                                    defaultValue={this.state.documentRef}
                                                    multiline
                                                    onChangeText={(name, text) => {
                                                        this.setState({ documentRef: text });
                                                    }}
                        ...
                        <View style={styles.footerDiv}>
                            <View style={styles.footerButtonsRow}>
                                <View style={{ width: width(30) }}>
                                    <TouchableOpacity
                                        onPress={() => debounce(this.setState({ dialogVisible: true }), 700)}
                                        style={styles.footerButton}>
                                        <Icon name="refresh-ccw" size={22} color="#ffffff" />
                                ...
                                <View style={{ width: width(30) }}>
                                    <TouchableOpacity
                                        ...
                                        style={[
                                            ...
                                            this.state.startVoice ? styles.footerVoiceButtonActive : null,
                                        ]}>
                                        <Icon
                                            name={this.state.startVoice === true ? 'assistive-listening-systems' : 'mic'}
                                            size={22}
                                            color={this.state.startVoice ? 'white' : '#ffffff'}
                                        />
                                        <Text style={styles.footerActionText}>{'Voice'}</Text>
                                    </TouchableOpacity>
                                </View>
                                <View style={{ width: width(30) }}>
