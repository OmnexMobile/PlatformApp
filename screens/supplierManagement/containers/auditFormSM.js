...
class AuditForm extends Component {
    ...
    componentWillMount() {
        ...
        if (this.props?.route?.params?.SpeechCommand) {
            ...
            } else if (this.props?.route?.params?.SpeechCommand == 'Reference') {
    ...
    displayNCSync(ncofiRecords) {
        ...
        if (this.props.data.audits.isAuditing === true && this.state.AuditID === this.props.route.params.AuditID) {
            this.setState({ notifyRed: true });
        }
    }
                                ...
                                if (audits[i].Listdata[j].Modified == true && audits[i].Listdata[j].FormId == FormIds[kk]) {
                                    ...
                                    if (
                                        ...
                                        auditCheckPoints.push({
                                            ...
                                            Status:
                                                audits[i].Status == '' || audits[i].Status == undefined || audits[i].Status == null
                                                    ? 0
                                                    : parseInt(audits[i].Status),
    ...
    handleNCOFIConnection() {
        ...
        for (var i = 0; i < dataArr.length; i++) {
            console.log('dataArr[i].AuditID === this.state.AuditID' + dataArr[i].AuditID + '' + this.state.AuditID);
                ...
                for (var j = 0; j < dataArr[i].Pending.length; j++) {
                    ...
                    if (dataArr[i].Pending[j].Category === 'NC') {
                        ...
                        formRequest.push({
                            ...
                            CategoryId: dataArr[i].Pending[j].categoryDrop ? dataArr[i].Pending[j].categoryDrop.id : 0,
                            ...
                            //   : 0,
                            ResponsibilityUser:
                                dataArr[i].Pending[j].userDrop && dataArr[i].Pending[j].userDrop.id ? dataArr[i].Pending[j].userDrop.id : 0,
                        ...
                        formRequest.push({
                            ...
                            AuditStatus:
                                dataArr[i].Pending[j].auditstatus === ''
                                    ...
                                    ? this.props.route.params.datapass
                                    : parseInt(dataArr[i].Pending[j].auditstatus),
                            RequestedBy: dataArr[i].Pending[j].requestDrop ? dataArr[i].Pending[j].requestDrop.id : 0,
                            NonConformity: dataArr[i].Pending[j].OFI,
                            FormId: dataArr[i].Pending[j].Formid === '' ? 0 : parseInt(dataArr[i].Pending[j].Formid),
                            SiteId: dataArr[i].Pending[j].SiteID,
                            ChecklistId: dataArr[i].Pending[j].ChecklistTemplateId === '' ? 0 : parseInt(dataArr[i].Pending[j].ChecklistTemplateId),
                            ElementID: dataArr[i].Pending[j].selectedItems ? dataArr[i].Pending[j].selectedItems.join(',') : 0,
                            ResponsibilityUser: dataArr[i].Pending[j].userDrop ? dataArr[i].Pending[j].userDrop.id : 0,
                            NCIdentifier: dataArr[i].Pending[j].ncIdentifier === undefined ? '' : dataArr[i].Pending[j].ncIdentifier,
    ...
    updateFormUploadDetails = () => {
        ...
        for (var p = 0; p < auditRecordsOrg.length; p++) {
            ...
            auditRecords.push({
                ...
                Status:
                    auditRecordsOrg[p].Status == '' || auditRecordsOrg[p].Status == undefined || auditRecordsOrg[p].Status == null
                        ? 0
                        : auditRecordsOrg[p].Status,
    ...
    viewDocument(fileData, fileName) {
        ...
        if (Platform.OS == 'android') {
            var path = dirs.DownloadDir.replace('/Download', '') + '/Android/data/com.omnex.suppliermanagement/cache/' + fileName.replace(/\s/g, '-');
        } else {
        ...
        return (
            <View style={styles.wrapper}>
                ...
                {this.state.uploadSpeed && (
                    <Text style={{ fontSize: 14, color: 'green', marginVertical: 4 }}>🔼 Upload Speed: {this.state.uploadSpeed}</Text>
                )}
