...
//assets
import styles from '../../auditPro/styles/AuditProDashboardStyle';
...
import AuditDashboardFooter from '../../auditPro/components/AuditDashboardFooter';
...
class AuditProDashboard extends Component {
  ...
  async getAuditLists() { 
    ...
    var token = this.state.token || this.state.currentUserData?.accessToken;
    var userId = this.state.userId || this.state.currentUserData?.userId;
  ...
  getSessionValues = () => {
    ...
    try {
      ...
      // const SITEID = this.props.data.audits.siteId;
      const TOKEN = this.props?.data?.audits?.token || this.state.currentUserData?.accessToken;
      const USERID = this.props?.data?.audits?.userId || this.state.currentUserData?.userId;
      const SITEID = this.props?.data?.audits?.siteId || this.state.currentUserData?.siteId;
      if (TOKEN !== null) {
        this.setState(
          {
            token: TOKEN,
  ...
  getAuditlist = (startDate, endDate) => {
    ...
    NetInfo.fetch().then(netState => {
      if (netState.isConnected) {
        ...
        var token = this.state.token || this.state.currentUserData?.accessToken;
        var userId = this.state.userId || this.state.currentUserData?.userId;
        var siteId = this.state.siteId || this.state.currentUserData?.siteId;
        var filterId = this.state.filterId;
        var pageSize = 10;
        var GlobalFilter =
          this.state.AuditSearch === undefined ? '' : this.state.AuditSearch;
      ...
      <View style={styles.mainContainer}>
        ...
        {this.state.showMyAllAudits ? (
          <View style={styles.showMyAll}>
            <TouchableOpacity
              onPress={() => this.props.navigation.navigate(ROUTES.ALLTABAUDITLIST_SM)}>
          ...
          <View style={{backgroundColor: 'white', marginTop: 6}}>
            <View style={styles.cardTitle}>
              ...
              {this.state.todaysactivity.length > 2 ? (
                <TouchableOpacity
                  onPress={() =>
                    this.props.navigation.push(ROUTES.ALLTABAUDITLIST_SM, {
                      ActiveTab: 'today',
                    })
                  }>
                  <Text style={styles.moreTxt}>{strings.more}</Text>
        ...
        <View style={styles.floatingDiv}>
          <TouchableOpacity
            onPress={() => this.props.navigation.navigate(ROUTES.VOICE_RECOGNITION)}
      ...
      <ImageBackground source={Images.DashboardBG} style={styles.header}>
        ...
        <View style={styles.welcomeTxtView}>
          <Text style={styles.welcomeTxt}>
            ...
            ( this.state.currentUserData?.userFullName ? (
              <Text style={[styles.welcomeTxt]} numberOfLines={1}>
                {(
                  this.state.currentUserData?.userFullName?.charAt(0).toUpperCase() +
                  this.state.currentUserData?.userFullName?.slice(1)
                ).length > 20
                  ? (
                      this.state.currentUserData?.userFullName?.charAt(0)?.toUpperCase() +
                      this.state.currentUserData?.userFullName?.slice(1)
        ...
        <View style={styles.headerIcon}>
          <TouchableOpacity
            ...
            onPress={() => this.props.navigation.navigate(ROUTES.USER_PREFERENCE)}>
  ...
  render_statusBar() {
    console.log('reach status--bar')
    return (
      <View style={styles.statusOuterView}>
        <TouchableOpacity
          style={[styles.statusView]}
          onPress={() =>
            this.props.navigation.navigate(ROUTES.AUDIT_DASHBOARD_LISTING_SM, {
              filterId: 2,
