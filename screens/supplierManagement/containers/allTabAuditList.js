...
//component
import CalendarAgenda from '../../auditPro/components/CalenderAgenda';
...
todayAudits() {
  ...
  loginCall = async () => {
    ...
    // if (data?.data?.Success == true) {
      console.log(
        'data value checking' + loginDetails?.data[0]?.SupplierManagementAccess,
      );
  ...
  async WriteFile(id) {
    ...
    if (Platform.OS == 'android') {
      ...
      setTimeout(() => {
        ...
        // write the file
        RNFS.writeFile(path, stringify, 'utf8').then(success => {
            // console.log('FILE WRITTEN!');
          this.setState(
            {
              progressVisible: false,
  ...
  componentWhenReceiveProps() {
    ...
    if (CurrentPage == ROUTES.ALLTABAUDITLIST_SM) {
      ...
      if (this.state.isMounted && this.state.isLazyLoadingRequired) {
        console.log('this.props.searchFlag == true----this.props.onRecieveSearchSubmit', this.props.searchFlag == true, this.props.onRecieveSearchSubmit)
        if (
