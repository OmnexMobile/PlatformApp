...
class CheckPointDemo extends Component {
                ...
                for (var i = 0; i < auditRecords.length; i++) {
                    if (auditRecords?.[i]?.AuditId == this.state.auditId) {
                        for (var j = 0; j < auditRecords?.[i]?.Listdata.length; j++) {
                            ...
                            if (
                                this.state.ChecklistTemplateId == auditRecords?.[i]?.Listdata?.[j]?.ParentId &&
                                // this.props.navigation.state.params.Formid ==
                                this.props?.route?.params?.Formid == auditRecords?.[i]?.Listdata?.[j]?.FormId
                            ) {
                                    ...
                                    if (this.props.data.audits.ncofiRecords) {
                                        for (var n = 0; n < this.props.data.audits.ncofiRecords.length; n++) {
                                            if (
                                                ...
                                                this.props?.route?.params?.AuditID == this.props.data.audits.ncofiRecords[n].AuditID
                                            ) {
                                                // console.log(
                ...
                if (checkPointsDetails.length > 0) {
                    ...
                    if (checkPoints && checkPoints.length > 0) {
                        for (var i = 0; i < checkPoints.length; i++) {
                            ...
                            if (templateId != null) {
                                var status = checkPoints?.[i]?.ncOFIStatus ?? checkPoints?.[i]?.ByteAttachment1;
                                if (status != null) {
                    ...
                    if (checkPointList && checkPointList.length > 0) {
                        for (var i = 0; i < checkPointList.length; i++) {
                            ...
                            if (templateId != null && checkPointList?.[i]?.scoreTypesData) {
                                scoreTypesByTemplate[templateId] = checkPointList[i].scoreTypesData;
                            }
                        }
                    }
                    ...
                    for (var i = 0; i < checkPointsDetails.length; i++) {
                        if (
                            ...
                            checkPointsDetails[i].scoreTypesData = scoreTypesByTemplate[checkPointsDetails[i].ChecklistTemplateId];
                        }

                        if (checkPointsDetails?.[i]?.show_nc_ofi_status == null && checkPointsDetails?.[i]?.scoreTypesData) {
                            var scoreTypesData = checkPointsDetails[i].scoreTypesData;
                            var scoreText = checkPointsDetails[i].Scoretext;
                            var scoreVal = checkPointsDetails?.[i]?.Score != null ? String(checkPointsDetails[i].Score) : '';
                            var byText = scoreTypesData.find(item => item.value == scoreText);
                            var byScore = scoreTypesData.find(item => item.id == scoreVal || item.value == scoreVal);
                            var derivedStatus = byText?.status != null ? byText.status : byScore?.status;
                        ...
                        if (checkPointsDetails?.[i]?.ncOFIStatus == null) {
                            if (checkPointsDetails?.[i]?.ByteAttachment1 != null) {
                                checkPointsDetails[i].ncOFIStatus = checkPointsDetails[i].ByteAttachment1;
                            } else {
                                var templateId = checkPointsDetails?.[i]?.ChecklistTemplateId;
                                if (templateId != null && ncofiStatusByTemplate[templateId] != null) {
                                    checkPointsDetails[i].ncOFIStatus = ncofiStatusByTemplate[templateId];
                                } else if (checkPointsDetails?.[i]?.show_nc_ofi_status != null) {
                                    checkPointsDetails[i].ncOFIStatus = checkPointsDetails[i].show_nc_ofi_status;
                                }
                            }
                        }
                    }
    ...
    componentWillReceiveProps(props) {
        ...
        const topRouteName = routes.length > 0 ? routes[routes.length - 1]?.name : null;
        const prevRouteName = routes.length > 1 ? routes[routes.length - 2]?.name : null;

        const isCameraFlowFromThisScreen = topRouteName === ROUTES.CAMERA_CAPTURE && prevRouteName === ROUTES.CHECKPOINT_DEMO_SM;

        console.log('--CheckpointDemoSM routes--->', topRouteName, prevRouteName, isCameraFlowFromThisScreen);
                        ...
                        if (ncofi_details[j].AuditID == checkPointsDetails[i].AuditId) {
                            ...
                            for (var k = 0; k < pending_ncofi.length; k++) {
                                if (
                                    ...
                                    pending_ncofi[k].ChecklistTemplateId == checkPointsDetails[i].ChecklistTemplateId
                                ) {
                            ...
                            console.log('NC_AVAILABLE_STATUS', checkPointsDetails[i].nc_available_status);
                            for (var k = 0; k < Uploaded_ncofi.length; k++) {
                                if (
                                    //Uploaded_ncofi[k].AuditID == checkPointsDetails[i].AuditId &&
                                    Uploaded_ncofi[k].ChecklistTemplateId == checkPointsDetails[i].ChecklistTemplateId
                                ) {
    ...
    updatecheckpointvalues_new = () => {
        ...
        this.setState(
            ...
            () => {
                ...
                for (var i = 0; i < checkPointsDetails.length; i++) {
                    if (checkPointsDetails[i].RemarkforNc == 1 || checkPointsDetails[i].RemarkforOfi == 1) {
                        if (checkPointsDetails[i].Remark == '') {
                            this.setState({ checkMandate: false }, () => {
                                //console.log('Please save remark manadatory fields.');
                            });
                        }
                    } else if (checkPointsDetails[i].AttachforNc == 1 || checkPointsDetails[i].AttachforOfi == 1) {
                        if (checkPointsDetails[i].AttachmentList.length == 0) {
                            this.setState({ checkMandate: false }, () => {
                                //console.log('Please save attach manadatory fields.');
                            });
                        }
                    } else if (checkPointsDetails[i].AttachforNc == 1 && checkPointsDetails[i].RemarkforNc == 1) {
                        if (checkPointsDetails[i].AttachmentList.length == 0 && checkPointsDetails[i].Remark == '') {
                            this.setState({ checkMandate: false }, () => {
                                //console.log('Please save attach manadatory fields.');
                            });
                        }
                    } else if (checkPointsDetails[i].AttachforOfi == 1 && checkPointsDetails[i].RemarkforOfi == 1) {
                        if (checkPointsDetails[i].AttachmentList.length == 0 && checkPointsDetails[i].RemarkforOfi == '') {
                            this.setState({ checkMandate: false }, () => {
                                //console.log('Please save attach manadatory fields.');
                            });
                        }
                                        ...
                                        if (auditRecordsOrg[p].Listdata.length > 0) {
                                            for (var q = 0; q < auditRecordsOrg[p].Listdata.length; q++) {
                                                ...
                                                listDataArr.push({
                                                    ...
                                                    ncOFIStatus:
                                                        auditRecordsOrg[p].Listdata[q].ncOFIStatus ?? auditRecordsOrg[p].Listdata[q].ByteAttachment1,
                                        ...
                                        for (var j = 0; j < checkPointsDetails.length; j++) {
                                            if (
                                                ...
                                                listDataArr[i] = {
                                                    ...
                                                    ncOFIStatus: checkPointsDetails[j].ncOFIStatus ?? listDataArr[i].ncOFIStatus,
    ...
    popupModal(checkPointDetail, checklist) {
        ...
        console.log('checkPointDetailpopup', checkPointDetail);
        console.log('checklistpopup', checklist);
        ...
        const checkpointItem = (this.state.checkpointList || []).find(item => {
            if (useFormId) {
                return item.ChecklistTemplateId == templateId && item.FormID == formId;
            }
        ...
        const ncOfiStatusFromScoreTypes = (() => {
            ...
            const scoreTypesData = checkPointDetail?.scoreTypesData || checkpointItem?.scoreTypesData;
            if (!scoreTypesData || scoreTypesData.length === 0) {
                return undefined;
            }
            const byScoreText = scoreTypesData.find(item => item.value == checkPointDetail?.Scoretext);
            if (byScoreText?.status != null) {
                return byScoreText.status;
            }
            const scoreVal = checkPointDetail?.Score != null ? String(checkPointDetail.Score) : '';
            const byScore = scoreTypesData.find(item => item.id == scoreVal || item.value == scoreVal);
        ...
        console.log('checkPointDetailpopup2ncOfiStatus', ncOfiStatus);
        this.setState(
            {
                ...
                for (var i = 0; i < auditRecords.length; i++) {
                    ...
                    if (this.state.ncofiPassAuditId == auditRecords?.[i]?.AuditId) {
                        ...
                        if (uploadedData.ResponsibilityId) {
                            ...
                            for (var j = 0; j < dropdowns.Users.length; j++) {
                                if (uploadedData.ResponsibilityId == dropdowns.Users[j].userid) {
                                    userObj = {
    ...
    clearCheckpoints = () => {
        this.setState({ isSaving: true, dialogVisibleReset: false, deleteallattachment: 1 }, () => {
            var checkPointsDetails = [];
            var index = 0;
                            ...
                            {this.props.data.audits.smdata !== 2 && this.props.data.audits.smdata !== 3 ? (
                                <View style={styles.statistics}>
                                    <View style={styles.statCard1}>
                                        <Text numberOfLines={1} style={{ fontSize: 14, fontFamily: 'OpenSans-Regular' }}>
                                    ...
                                    <View style={styles.statCard3}>
                                        <Text style={{ fontSize: 14, fontFamily: 'OpenSans-Regular' }}>{'Asterisk Questions'}</Text>
                                        <Text
                                            style={{
                                                fontSize: Fonts.size.h5,
                                                fontFamily: 'OpenSans-Regular',
                                    ...
                                    <TouchableOpacity
                                      ...
                                      <Text
                                        ...
                                        NC/OFI8
                                                                    ...
                                                                    this.state.checkPointsDetails[index].Score == -2 ? null : (
                                                                        <>
                                                                            {item.scoreType == 3 &&
                                                                            ...
                                                                            (this.state.checkPointsDetails[index].show_nc_ofi_status == 1 ||
                                                                                this.state.checkPointsDetails[index].show_nc_ofi_status == 2 ||
                                                                                this.state.checkPointsDetails[index].show_nc_ofi_status == 3 ||
                                                                                this.state.checkPointsDetails[index].show_nc_ofi_status == 4) &&
                                                                            this.state.TemplateID !== 5 &&
                                                                            this.state.ischeckLPA !== true ? null : (
                                                                                <>
                                                                                    <View
                                                                                        style={{
                                                                                            width: '100%',
                                                                                    ...
                                                                                    this.props.data.audits.smdata !== 3 ? (
                                                                                        <Dropdown
                                                                                            ...
                                                                                            onChangeText={(value, index, data) => {
                                                                                                ...
                                                                                                const text = value == 'Please select' ? '-3' : value;
                                                                                            ...
                                                                                            if (
                                                                                                ...
                                                                                                for (var j = 0; j < data.length; j++) {
                                                                                                    if (value == data[j].value) {
                                                                                                        ...
                                                                                                        checkPointsDetails[i].FailureCategoryId =
                                                                                                            data[j].id;
                                                                                                        checkPointsDetails[i].Modified = true;
                                                                                                        checkPointsDetails[i].FailureCategoryId =
                                                                                                            value;
                                                                                            ...
                                                                                            if (
                                                                                                ...
                                                                                                for (var j = 0; j < data.length; j++) {
                                                                                                    if (value == data[j].value) {
                                                                                                        ...
                                                                                                        checkPointsDetails[i].FailureReasonId =
                                                                                                            data[j].id;
                                                                                                        checkPointsDetails[i].Modified = true;
                                                                                                        checkPointsDetails[i].FailureReasonId = value;
                                                                                                    }
                                                                                                }
                                                                                            }
                                                                                        }
                                                                        ...
                                                                        <View style={{ width: '100%' }}>
                                                                                                ...
                                                                                                if (
                                                                                                    ...
                                                                                                    for (var j = 0; j < data.length; j++) {
                                                                                                        if (value == data[j].value) {
                                                                                                            console.log('Approach dropdown', value);
                                                                                                            checkPointsDetails[i].Modified = true;
                                                                                                            checkPointsDetails[i].immediateAction =
                                                                                                                value;
                                                                                                        }
                                                                                                    }
                                                                                                }
                                                                            ...
                                                                            <TextInput
                                                                                ...
                                                                                placeholderTextColor={
                                                                                    item.RemarkforNc ||
                                                                                    item.RemarkforOfi == 1 ||
                                                                                    this.state.checkPointsDetails[index].Values !==
                                                                                        this.state.checkPointsDetails[index].RadioValue
                                                                                        ? 'red'
                                                                                        : '#A9A9A9'
                                                                                }
