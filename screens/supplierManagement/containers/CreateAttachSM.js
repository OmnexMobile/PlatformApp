          ...
          async () => {
            ...
            for (var i = 0; i < auditRecords.length; i++) {
              if (this.state.AuditID === auditRecords[i].AuditId) {
                ...
                AuditProgramOrder: auditRecords?.[i]?.AuditProgOrder == undefined ? AUDITPROGORDER : auditRecords?.[i]?.AuditProgOrder,
 ...
 var userDetails = await this.getAccessToken();
  ...
  render() {
    ...
    const dropdownBackgroundColor =
      theme && theme.mode && theme.mode.backgroundColor ? theme.mode.backgroundColor : '#fff';
    const dropdownBorderColor =
      theme && theme.mode && theme.mode.borderColor ? theme.mode.borderColor : '#A6A6A6';
