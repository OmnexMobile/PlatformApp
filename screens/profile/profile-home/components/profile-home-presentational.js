...
const ProfileHomePresentational = ({ menus, navigation, versionDetails, handleLogout, profileData }) => {
    const { theme } = useTheme();
    return (
        <Content noPadding>
            <TouchableOpacity
                onPress={() => navigation.goBack()}
                style={{ position: 'absolute', left: SPACING.NORMAL, top: SPACING.NORMAL, zIndex: 1 }}>
                <IconComponent name={'arrowleft'} type={ICON_TYPE.AntDesign} color={theme.colors.primaryThemeColor} size={25} />
            </TouchableOpacity>
            <TouchableOpacity
                onPress={() => navigation.navigate(ROUTES.SETTINGS)}
                style={{ position: 'absolute', right: SPACING.NORMAL, top: SPACING.NORMAL, zIndex: 1 }}>
                                ...
                                <View>
                                    ...
                                    <LabelValue
                                        {...{
                                            iconType: ICON_TYPE.Feather,
                                            iconName: 'link',
                                            value: 'http://1.22.172.236/APQPApi/',
                                        }}
                                    />
                                </View>
                            </View>
