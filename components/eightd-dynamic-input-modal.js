...
import { APP_VARIABLES, EIGHTD_FORM_VALUE_TYPE, FONT_TYPE, ICON_TYPE, INPUTS_CONSTANTS } from 'constants/app-constant';
...
import RootCauseCategoryPickerComponent from './root-cause-category-picker';
...
const ADD_CAPABILITIES = [
    ...
    'ERContainmentAction_ActionDescription',
...
const STATUS_DROPDOWNS = [
    ...
    {
        Key: 'CorrectiveAction_Responsibility',
        Value: 'ResponsibilityMember',
        Field1: 'ResponsibilityMemberName',
        Field2: 'ResponsibilityMemberId',
    },
    {
        Key: 'Disposition_SupplierDispositionId',
        Value: 'Disposition',
        Field1: 'DispositionName',
        Field2: 'Id',
    },
    {
        Key: 'Disposition_ChargeBackAlignmentID',
        Value: 'ChargeBackAlignment',
        Field1: 'Description',
        Field2: 'Id',
    },
    {
        ...
        Field1: 'RootCauseCategoryName',
        Field2: 'RootCauseCategoryId',
    },
    {
        Key: 'PreventReocurrance_HorDeployStatus',
        Value: 'ActionStatus',
        Field1: 'StatusName',
        Field2: 'StatusId',
    },
    {
        Key: 'PreventReocurrance_SysProbStatus',
        Value: 'ActionStatus',
        Field1: 'StatusName',
        Field2: 'StatusId',
    },
    ...
    {
        ColumnDefinition: 'ERActionUsage_Responsibility',
        Value: 'ResponsibilityMember',
        Field1: 'ResponsibilityMemberName',
        Field2: 'ResponsibilityMemberId',
    },
...
const WrapperDropDown = ({ input, onChange, dropdownData, setSelectedData, selectedData, ConcernID }) => {
    ...
    const TypeId = selectedData?.Key || dropdownData?.NodeName;

    const getData = async newValueId => {
        // const isStatus = dropdownData?.Key === 'InterimAction_Status';
        try {
            const isDefaultDD = STATUS_DROPDOWNS.find(dd => (dd?.Key || dd?.ColumnDefinition) === (dropdownData?.Key || dropdownData?.NodeName));
    ...
    const handleSaveNewValue = async () => {
        ...
        try {
            ...
            const formData = new FormData();
            formData.append('ActionUsageId', dropdownData?.UsageId || dropdownData?.UsageID || 0);
    ...
    return (
        <View
            ...
            <View
                ...
                {ADD_CAPABILITIES.includes(dropdownData?.NodeName) && (
                    <TouchableOpacity
                        style={{
...
const EightDDynamicInputModal = ({ selectedData, setSelectedData, ConcernID, getDynamicFormData }) => {
    ...
    const handleSubmitFormData = async () => {
        ...
        try {
            ...
            if (data?.Type === INPUTS_CONSTANTS.FILE_UPLOAD) {
                const URL = selectedData?.APIEndPoint || API_URL.SAVE_EIGHTD_ATTACHMENT;
                const fileName = selectedData?.Value?.fileName || uuid.v4();
                const base64Content = selectedData?.Value?.base64;
                ...
                const payload = selectedData?.APIEndPoint ? formData : request;
                ...
                if (data?.type === EIGHTD_FORM_VALUE_TYPE.STATIC) {
                    ...
                    console.log('🚀 ~ selectedData?.ColumnNames:', selectedData?.ColumnNames);

                    formData.append(selectedData?.NodeName, formatMomentObject(selectedData?.Value));
                    if (selectedData?.rowData?.Typeid) {
                        formData.append('Typeid', selectedData?.rowData?.Typeid);
                    }
    ...
    const renderInputComponent = () => {
        switch (data?.Type) {
            ...
            case INPUTS_CONSTANTS.ROOT_CAUSE_CATEGORY_PICKER:
                return (
                    <RootCauseCategoryPickerComponent
                        {...{
                            label: data?.Label,
                            name: data?.ColumnDefinition,
                            value: data?.Value,
            ...
            case INPUTS_CONSTANTS.RADIO_BUTTON:
                return (
                    <RadioButton
                        ...
                        {...{
                            ...(data?.Value !== '' && {
                                value: ['Yes', 'YES', '1', 1].includes(data?.Value) ? '1' : ['NA', '2', 2].includes(data?.Value) ? '2' : '0',
                            }),
                        }}
            ...
            case INPUTS_CONSTANTS.TEXT_AREA:
                return (
                    <TextAreaWithLabel
                        numberOfLines={7}
                        noPadding
            ...
            case INPUTS_CONSTANTS.NOTIFY_USER:
                return <ButtonComponent>Send Notification</ButtonComponent>;
    ...
    return (
        <ModalComponent modalVisible={true} onRequestClose={closeModal} modalBackgroundColor={COLORS.transparentGrey} transparent>
            ...
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                <View style={{ flex: 1, backgroundColor: COLORS.transparentGrey }}></View>
                <View
                    style={{
                        flex: 9,
                    ...
                    <View style={{ padding: SPACING.NORMAL, flex: 1 }}>
                        ...
                        <View style={{ padding: 0 }}>
                            {selectedData?.Type !== INPUTS_CONSTANTS.NOTIFY_USER && (
                                <ButtonComponent loading={updatingValue} onPress={handleSubmitFormData}>
                                    Submit
                                </ButtonComponent>
                            )}
                        </View>
