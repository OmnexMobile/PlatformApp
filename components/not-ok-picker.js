...
const NotOKPicker = ({
    ...
    const getNotOKPicker = async () => {
        try {
            setLoading(true);
            const formData = new FormData();
            formData.append(APP_VARIABLES.CONCERN_ID, ConcernID);
            formData.append(APP_VARIABLES.Type, 2);
            formData.append(APP_VARIABLES.FileId, concernDetails?.NotOkId);
            const { Data } = await postAPI(`${API_URL.GET_NOT_OK_PICKER}`, formData);
            ...
            setImage(image);
            setComments(concernDetails?.NotOkComments);
            setLoading(false);
        } catch (err) {
            setLoading(false);
    ...
    const handleDeleteNotOKPicker = async FileId => {
        try {
            setLoading(true);
            const formData = new FormData();
            ...
            if (Data?.Success) {
                console.log('🚀 ~ handleDeleteNotOKPicker ~ Data:', Data);
                setLoading(false);
                getNotOKPicker();
            }
        } catch (err) {
    ...
    const handleClickDeleteImage = (item, id) => {
        if (item?.FileId) {
            handleDeleteNotOKPicker(item?.FileId);
        } else {
    ...
    useEffect(() => {
        if (!Boolean(ConcernID) || isEditPage || concernDetails?.StatusID === STATUS_CODES.DraftConcern) {
            ...
            handleNotOKPicker?.(formattedContent);
        }
                        ...
                        <View
                            ...
                            <View
                                ...
                                <InputWithLabel
                                    ...
                                    value={comments}
                                    inputStyle={{
                                        ...(!editable && { backgroundColor: theme.mode.disabledBackgroundColor, paddingLeft: SPACING.SMALL }),
                                    }}
                                />
                            </View>
                        </View>
