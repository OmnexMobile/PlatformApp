...
const Modal = ({
    ...
    const handleDynamicInputChange = (label, value) => {
        ...
        setEstimationDetails(estimationDetails => ({
            ...estimationDetails,
            [label]: value,
        }));
    ...
    const dropDownDefaultValue = estimationDetails?.CurrencyId || concernDetails?.CurrencyId || '';
    ...
    const defaultInputs = useMemo(
        ...
        [dropdownList?.data, dropDownDefaultValue],
    );
...
const InputContent = ({ label, required, value, setModalVisible, editable, currencySymbol, defaultCurrencyWhileLoad }) => {
    const { theme } = useTheme();
    return (
        <>
            ...
            <TouchableOpacity
                ...
                <TextComponent
                    ...
                    {value || '0'} {currencySymbol || defaultCurrencyWhileLoad || ''}
                </TextComponent>
            </TouchableOpacity>
        </>
    );
...
const EstimationPickerComponent = ({
    ...
    const defaultCurrencyWhileLoad = dropdownList?.data?.find(currency => currency?.value === concernDetails?.CurrencyId)?.label || '';
    ...
    const selectedCurrency = dropdownList?.data?.find(currency => currency?.value === estimationDetails?.CurrencyId)?.label;
